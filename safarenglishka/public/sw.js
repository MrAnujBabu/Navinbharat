const CACHE_NAME = 'safar-english-ka-v1';
// Precache only assets that are ACTUALLY used on first paint. The previously
// listed /brand/safar-mark.png (53 KB) is orphaned in code; the
// real brand mark used by the app shell + <head> preload is /brand/safar-mark.png.
const STATIC_ASSETS = [
  '/',
  '/icons/icon-192x192.png',
  '/icons/icon-512x512.png',
  '/brand/safar-mark.png',
];
  ...
  if (request.mode === 'navigate') {
    ...
    event.respondWith(
      fetch(request).catch(async () => {
        const cached = (await caches.match('/')) ?? (await caches.match('/dashboard'));
