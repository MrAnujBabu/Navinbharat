...
import logo from "../assets/brand/safar-logo.png";
...
const Signup = () => {
  ...
  useEffect(() => {
    if (!authLoading && isAuthenticated) {
      toast.success("Account created! Welcome to Safar English Ka!");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (serverCheck?.blocked) {
        const reason = typeof serverCheck.reason === "string" && serverCheck.reason.trim()
          ? serverCheck.reason
    ...
    try {
      ...
      if (result.error) {
        const rawMsg = typeof result.error?.message === "string" ? result.error.message : "";
        const msg = rawMsg.toLowerCase();
        const isNetwork = /network|fetch|timeout|abort|timed|connection/i.test(rawMsg);
        ...
        if (msg.includes("already registered") || msg.includes("already been registered") || msg.includes("user already")) {
          ...
          setErrorMessage(rawMsg && rawMsg !== "{}" ? rawMsg : "Failed to create account. Please try again.");
        }
        return;
      }
      ...
      <div className="hidden lg:flex flex-1 bg-secondary items-center justify-center p-12">
        <div className="max-w-lg text-center text-secondary-foreground">
          <div className="w-24 h-24 mx-auto mb-8 bg-secondary-foreground/20 rounded-3xl flex items-center justify-center">
            <img src={logo} alt="Safar English Ka" width={64} height={64} loading="eager" decoding="async" className="h-16 w-16 rounded-xl" />
          </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-5 sm:px-8 py-8 sm:py-12 lg:px-16">
        <div className="mx-auto w-full max-w-md">
          <Link to="/" className="flex items-center gap-3 mb-8 lg:hidden">
            <img src={logo} alt="Safar English Ka" width={48} height={48} loading="eager" decoding="async" className="h-12 w-12 rounded-xl" />
            <span className="font-bold text-2xl text-foreground">Safar English Ka</span>
          </Link>

          <h1 className="text-3xl font-bold text-foreground mb-2">Create Account</h1>
          <p className="text-muted-foreground mb-8">
            Join Safar English Ka today - Get instant access to free courses!
          </p>
