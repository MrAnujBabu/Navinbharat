...
import logoIcon from "../../assets/sarthi-avatar.webp"; // Sarthi guru avatar
import nbLogo from "../../assets/brand/safar-logo.png"; // NB brand logo
...
const WELCOME_MSG = "👋 नमस्ते! मैं **Naveen Sarthi Agent** हूँ – आपका intelligent 24×7 learning companion। 🤖✨\n\nमैं आपकी मदद कर सकता हूँ:\n- 📚 **Lectures, PDFs, DPPs** recommend करूँगा आपके course से\n- 📝 **Quiz** mode — मैं MCQs दूँगा और score बताऊँगा\n- 🎯 **Study tips**, mnemonics और exam strategy\n- 🔗 **Content links** जो directly website पर play होंगे\n- 🖼️ **Image doubt** upload करें और मैं समझाऊँगा!\n\nआज मैं आपके लिए क्या कर सकता हूँ?";
...
const ChatWidget = forwardRef<HTMLDivElement>(() => {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const reason = data?.error || raw?.slice(0, 200) || `HTTP ${response.status}`;
      ...
      console.error("[ChatWidget] chatbot call failed:", reason);
      setMessages(prev => [...prev, {
        role: "assistant",
  ...
  const resetChat = () => {
    ...
    setMessages([{
      role: "assistant",
      content: "👋 नमस्ते! मैं **Naveen Sarthi Agent** हूँ – नई बातचीत शुरू करते हैं! आज मैं आपकी कैसे मदद कर सकता हूँ? 🤖🎓",
      timestamp: new Date(),
      id: "welcome-reset",
      feedbackGiven: null,
    }]);
  ...
  if (!user) {
    return (
      <div data-chat-widget="true" className="fixed bottom-20 right-4 z-50 md:bottom-6 md:right-6 flex flex-col items-end gap-2">
        {/* Login tooltip */}
        {showLoginTip && (
          <div
            className={cn(
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2 mb-2">
              <p className="font-semibold text-sm text-foreground">Naveen AI Agent 🤖</p>
            ...
            <p className="text-muted-foreground text-xs leading-relaxed mb-3">
              सीखने के लिए <strong className="text-foreground">Login करें</strong> और AI Agent से 24×7 बात करें।
            </p>
            {/* CTA */}
            <Link
              to="/login"
        ...
        <button
          ...
          aria-label="Login to chat with Naveen AI Agent"
        >
  ...
  return (
    <div data-chat-widget="true" className="contents">
      ...
      <button
        ...
        aria-label="Open Naveen AI Agent"
      >
        <img src={logoIcon} className="w-8 h-8 object-contain" alt="Naveen Sarthi" />
      </button>
          ...
          <div className="safe-area-top bg-background border-b border-border shrink-0">
            <div className="flex items-center gap-3 px-4 py-3">
              <div className="w-9 h-9 rounded-full overflow-hidden ring-1 ring-border bg-muted shrink-0 flex items-center justify-center">
                <img src={nbLogo} className="w-7 h-7 object-contain" alt="Safar English Ka" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-base text-foreground leading-tight truncate">Naveen Sarthi</p>
