-- Revoke EXECUTE from PUBLIC/anon/authenticated on internal-only SECURITY DEFINER + trigger functions.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname IN (
        'handle_new_user','handle_new_user_role',
        'update_updated_at_column','update_books_updated_at',
        ...
        'audit_security_policies','enforce_user_name_from_profile',
        'lock_submitted_quiz_attempt','sanitize_quiz_attempt_insert',
        'enforce_message_recipient_readonly','rate_limit_lead_insert',
        'complete_paid_enrollment','process_refund'
      )
  LOOP
...
-- Re-assert EXECUTE for client-callable helpers.
...
GRANT EXECUTE ON FUNCTION public.check_rate_limit_text(text, text, integer, integer) TO authenticated;
