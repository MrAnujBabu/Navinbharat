...
const DRIVE_UPSTREAM_TIMEOUT_MS = 120_000;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Do not forward upstream Content-Length. Deno/Fetch may transparently
    // decompress or normalize the body while keeping the original length; pdf.js
    // then aborts with "Content-Length header ... exceeds response Body". Let
    // the runtime compute framing for the relayed stream.
