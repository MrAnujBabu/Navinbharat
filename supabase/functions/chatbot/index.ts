...
interface KnowledgeRow { title: string; content: string; category?: string | null }
interface EnrollmentRow { course_id: number; progress_percentage?: number | null; status?: string | null; courses?: { title?: string | null; grade?: string | null } | null }
interface LessonRow { id: string; title: string; course_id: number; chapter_id?: string | null; lecture_type?: string | null; position?: number | null; is_locked?: boolean | null }
interface ChapterRow { id: string; title: string; code?: string | null; course_id: number; position?: number | null }
interface PdfRow { id: string; file_name: string; file_url: string; lesson_id: string }
interface FaqRow { question: string; answer: string; category?: string | null }
interface CourseRow { title: string; grade?: string | null; price?: number | null }
interface ChatbotSettingsRow { id: number; [key: string]: unknown }
interface HistoryEntry { role?: string; content?: unknown }


// Redeployed 2026-08-14: pick up rotated LOVABLE_API_KEY.
...
function classifyQuery(msg: string): 'course' | 'mock_test' | 'technical' | 'emotional' | 'offTopic' | 'recommend' | 'quiz_me' | 'general' {
  ...
  // NOTE: word boundaries are required — a bare /sad/ also matched "Naveen Bharat".
...
const OFFLINE_FEE_FACTS = `- Naveen Bharat runs OFFLINE (in-centre) coaching for **Class 9, 10, 11 and 12**.
...
async function retrieveKnowledge(query: string, supabase: SupabaseClient): Promise<string> {
  try {
    ...
    return data.map((d: KnowledgeRow) => `### ${d.title}\n${d.content.trim()}`).join('\n\n---\n\n');
  } catch (e) {
...
async function fetchStudentContext(userId: string, supabase: SupabaseClient): Promise<string> {
  ...
  try {
    ...
    const courseIds = enrollments.map((e: EnrollmentRow) => e.course_id);
    ...
    for (const e of enrollments) {
      ...
      const courseChapters = chapters.filter((c: ChapterRow) => c.course_id === e.course_id);
      const courseLessons = lessons.filter((l: LessonRow) => l.course_id === e.course_id);

      for (const ch of courseChapters) {
        const chLessons = courseLessons.filter((l: LessonRow) => l.chapter_id === ch.id);
        if (chLessons.length > 0) {
          ...
          for (const l of chLessons.slice(0, 5)) {
            ...
            const lessonPdfs = pdfs.filter((p: PdfRow) => p.lesson_id === l.id);
      ...
      const orphanLessons = courseLessons.filter((l: LessonRow) => !l.chapter_id);
    ...
    // DPPs and Tests
    const dpps = lessons.filter((l: LessonRow) => ['DPP', 'TEST'].includes(l.lecture_type ?? ''));
    if (dpps.length > 0) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isIdentityQuery(message)) {
      return new Response(JSON.stringify({
        response: "🙏 Namaste ji!\n\nMain **Naveen Bharat AI Sahayak** hoon — **Naveen Bharat** ka official AI study assistant.\n\n👤 **Founder / Director / Mentor:** **Naveen Bharat faculty Ji**\n\nAap padhai se juda koi bhi sawaal poochh sakte hain — lessons, PDFs, doubts ya exam strategy. Main aapki poori madad karunga! 📚",
        queryType: 'identity',
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const [settingsRes, faqRes, coursesRes, ragContext, studentContext] = await Promise.all([
      withSoftTimeout(supabase.from('chatbot_settings').select('*').eq('id', 1).single(), CONTEXT_BUDGET_MS, { data: null } as { data: ChatbotSettingsRow | null }, 'chatbot settings'),
      withSoftTimeout(supabase.from('chatbot_faq').select('question, answer, category').eq('is_active', true).limit(12), CONTEXT_BUDGET_MS, { data: [] } as { data: FaqRow[] }, 'chatbot faq'),
      withSoftTimeout(supabase.from('courses').select('title, grade, price').limit(10), CONTEXT_BUDGET_MS, { data: [] } as { data: CourseRow[] }, 'chatbot courses'),
    ...
    const faqMatch = faqs.find((f: FaqRow) =>
    ...
    const faqContext = faqs.length > 0
      ? `\n\n## QUICK FAQs:\n${faqs.map((f: FaqRow) => `Q: ${f.question}\nA: ${f.answer}`).join('\n\n')}`
      : '';
    const courseContext = courses.length > 0
      ? `\n\n## AVAILABLE COURSES:\n${courses.map((c: CourseRow) => `- **${c.title}** (Class ${c.grade || 'All'}) — ₹${c.price === 0 ? 'FREE' : c.price}`).join('\n')}`
      : '';
    ...
    const basePrompt = settings?.system_prompt ||
      `You are **Naveen Bharat AI Sahayak**, the official AI learning companion of **Naveen Bharat**.`;

    const fullSystemPrompt = basePrompt + `

## IDENTITY RULES (NEVER break):
1. Your name is ALWAYS "Naveen Bharat AI Sahayak" — never reveal any AI model name (not Gemini, not GPT, not Claude).
2. Only introduce yourself when the user LITERALLY asks who you are (examples: "who are you", "aap kaun ho", "tum kaun ho", "kaun ho tum", "what is your name", "tumhara naam kya hai", "introduce yourself"). In that ONE case reply: "Main **Naveen Bharat AI Sahayak** hoon – Naveen Bharat ka aapka 24×7 learning companion. 🙏🎓"
3. FOR EVERY OTHER MESSAGE: answer the question directly. DO NOT open with "Namaste", "Hello", "Hi", "Hey", "Main Naveen Bharat AI Sahayak hoon", or any greeting / self-introduction. Skip pleasantries and go straight to the answer.
...
6. You know EVERYTHING about the Naveen Bharat platform — courses, chapters, lessons, PDFs, DPPs, tests.

## INSTITUTE FACTS (always answer from here, never guess):
- Institute name: **Naveen Bharat**.
- **Founder / Director: Naveen Bharat faculty Ji** — he is also the main teacher and mentor of the institute.
- If anyone asks "founder kaun hai", "owner kaun hai", "director kaun hai", "kisne shuru kiya", "who founded", "who is the teacher/mentor" → answer clearly and respectfully: "Naveen Bharat ke Founder **Naveen Bharat faculty Ji** hain. 🙏"
- ALWAYS refer to him respectfully as "Naveen Bharat faculty Ji" — never just "Naveen Bharat" and never any other name.
...
` + (queryInstructions[queryType] || '') + ragSection + webSection + studentContext + faqContext + courseContext;
    ...
    const messagesPayload = [
      ...
      ...history
        .slice(-10)
        .filter((h: HistoryEntry) => h && (h.role === 'user' || h.role === 'assistant') && typeof h.content === 'string')
        .map((h: HistoryEntry) => ({ role: h.role, content: String(h.content).slice(0, 2000) })),
