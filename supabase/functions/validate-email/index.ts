...
serve(async (req) => {
  ...
  try {
    // Per-IP rate limiting to prevent unauthenticated abuse.
    const ip =
    ...
    const { data: allowed, error: rlErr } = await admin.rpc("check_rate_limit_text", {
      _bucket: "validate_email",
      _identifier: ip,
      _max: 20,
      _window_seconds: 60,
    });
    if (rlErr) console.error("validate-email rate limit rpc error:", rlErr);
    if (allowed === false) {
      return new Response(JSON.stringify({ blocked: true, reason: "Rate limit exceeded" }), {
        status: 429,
