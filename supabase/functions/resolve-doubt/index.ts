...
serve(async (req) => {
  ...
  try {
    ...
    if (!isLessonChat) {
      ...
      if (words.length > 0) {
        ...
        if (kbData && kbData.length > 0) {
          ragContext = "\n\nRelevant platform knowledge:\n" + kbData.map((d: { title: string; content: string }) => `- ${d.title}: ${d.content.slice(0, 300)}`).join("\n");
        }
      }
    }
    ...
    // Tracks whether the final prompt really carries grounding (transcript from
    // client/cache/fetch, description or overview). Used for the temperature
    // choice below — a cache-resolved transcript must not be treated as ungrounded.
    let grounded = false;
    ...
    if (isLessonChat) {
      ...
      systemPrompt =
`You are an Academic Doubt Solver AI for the Naveen Bharat coaching platform (English grammar, literature, and language teaching).
...
${ctx || "(no lesson context provided)"}
    ...
    const ungrounded = isLessonChat && !grounded;
