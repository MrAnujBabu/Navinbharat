...
async function discoverTracks(youtubeId: string): Promise<Array<{ url: string; lang: string }>> {
  try {
    ...
    return tracks
      .filter((t: { baseUrl?: unknown; languageCode?: unknown }) => t && typeof t.baseUrl === "string")
      .map((t: { baseUrl?: unknown; languageCode?: unknown }) => ({ url: t.baseUrl as string, lang: (t.languageCode as string) || "unk" }));
  } catch {
    return [];
  }
}
