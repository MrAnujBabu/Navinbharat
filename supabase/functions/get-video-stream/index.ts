...
interface PipedInstance { api_url?: string; uptime_24h?: number }
interface PipedStream { url?: string; mimeType?: string; format?: string; videoOnly?: boolean; quality?: string }
interface PipedData { videoStreams?: PipedStream[] }
interface InvidiousFormat { url?: string; type?: string; qualityLabel?: string; quality?: string; container?: string; encoding?: string }
interface InvidiousData { formatStreams?: InvidiousFormat[]; adaptiveFormats?: InvidiousFormat[] }
interface YTFormat { url?: string; signatureCipher?: string; cipher?: string; mimeType?: string; qualityLabel?: string; quality?: string; audioQuality?: string; audioChannels?: number }
interface YTStreamingData { formats?: YTFormat[]; adaptiveFormats?: YTFormat[] }
interface YTPlayerResponse { playabilityStatus?: { status?: string }; streamingData?: YTStreamingData }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data)) {
        const apis = data
          .filter((d: PipedInstance) => d.api_url && (d.uptime_24h ?? 0) > 90)
          .sort((a: PipedInstance, b: PipedInstance) => (b.uptime_24h || 0) - (a.uptime_24h || 0))
          .map((d: PipedInstance) => {
...
function parsePipedStreams(data: PipedData): StreamInfo[] {
...
function parseInvidiousStreams(data: InvidiousData): StreamInfo[] {
  const streams: StreamInfo[] = [];
