...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const q of questions) {
      ...
      const rawOptions = (q as QuestionRow).options;
      const opts = Array.isArray(rawOptions)
        ? rawOptions
        : (rawOptions ? Object.values(rawOptions) : []);
      if (isAnswerCorrect(userAnswer, q.correct_answer, (q as QuestionRow).question_type, opts as string[])) {
        score += q.marks ?? 4;
      } else {
    ...
    const totalMarks = quiz.total_marks || questions.reduce((s: number, q: { marks?: number | null }) => s + (q.marks ?? 4), 0);
