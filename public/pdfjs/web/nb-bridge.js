...
//
/** @typedef {"odd"|"even"|"all"|"custom"|"route"} DwellParity */
/**
...
(function () {
  ...
  var DWELL_MIN_SECONDS = 5;
  var DWELL_MAX_SECONDS = 120;
  var MAX_LIST_LENGTH = 500;
  var dwellCfg = { enabled: false, parity: "odd", pages: [], route: [], loopRoute: false, seconds: 30 };
  ...
  function routeActive() {
    return dwellCfg.enabled && dwellCfg.seconds > 0 &&
      dwellCfg.parity === "route" && dwellCfg.route.length > 0;
  }
    ...
    dwellTops = Array.prototype.slice
      ...
      .map(function (p, i) {
        return {
          ...
          top: p.getBoundingClientRect().top - rootTop,
        };
      })
      .sort(function (a, b) { return a.top - b.top; });
  }

  function routeTargetTop(c, now) {
    ...
    for (var i = 0; i < dwellTops.length; i++) {
      if (dwellTops[i].page === wanted) return dwellTops[i].top;
    }
    return null;
  }
  ...
  window.addEventListener("message", function (e) {
    ...
    if (data.type === "nb-autoscroll-tick") {
      ...
      if (c) {
        ...
        var target = null;
        if (isRoute) {
          target = routeTargetTop(c, tickNow);
          if (target !== null) {
        ...
        if (isRoute) {
          ...
          var dir = dy < 0 ? -1 : 1;
          if (dir !== dwellDir) { dwellDir = dir; dwellPage = null; }
          if (tickNow - dwellTopsAt > 500 || !dwellTops.length) {
            dwellTopsAt = tickNow;
            measureDwellTops(c);
          }
          var lo = Math.min(prevPos, pos);
          var hi = Math.max(prevPos, pos);
          var hits = dwellTops.filter(function (p) {
            return p.top > lo + 0.001 && p.top <= hi + 0.001 && dwellMatches(p.page);
          });
          var crossed = dir < 0 ? hits[hits.length - 1] : hits[0];
          if (crossed && dwellPage !== crossed.page) {
