...
(function () {
  ...
  // ── Zoom policy: 100% (page-width) is the floor ─────────────────────────
  // Mirrors src/lib/pdfZoom.ts for the canvas reader. Zooming out below
  // page-width renders the document too small to read on a phone, so the
  // toolbar zoom controls are hidden and any smaller scale snaps back.
  (function enforceZoomFloor() {
    try {
      var style = document.createElement("style");
      style.textContent =
        "#zoomIn,#zoomOut,#zoomInButton,#zoomOutButton,#scaleSelectContainer{display:none !important}";
    ...
    function attach() {
      var app = window.PDFViewerApplication;
      var bus = app && app.eventBus;
      if (!bus || !app.pdfViewer) return setTimeout(attach, 200);
      function baseline() {
        if (floor != null) return floor;
        try {
          var prev = app.pdfViewer.currentScale;
          app.pdfViewer.currentScaleValue = "page-width";
          floor = app.pdfViewer.currentScale;
          if (prev > floor) app.pdfViewer.currentScale = prev;
        } catch (_) { floor = null; }
        return floor;
      }
      bus.on("pagesloaded", function () { floor = null; baseline(); });
      bus.on("scalechanging", function (e) {
        if (fixing) return;
        var min = baseline();
        if (min == null || !e || typeof e.scale !== "number") return;
        if (e.scale < min - 0.005) {
          fixing = true;
          setTimeout(function () {
