...
export function useAutoScroll({ targetRef, iframeRef, docKey }: AutoScrollOptions) {
  ...
  const setSpeed = useCallback((s: number) => {
    ...
    const clamped = Math.max(0.02, Math.min(10, Math.round(s * 100) / 100));
  ...
  const start = useCallback(() => {
    ...
    if (useEl && el) {
      ...
      // Cached page tops (relative to the scroller's content) for repeat mode.
      let pageTops: { page: number; top: number }[] = [];
      let pageTopsAt = 0;
      const measurePages = () => {
        ...
        pageTops = Array.from(el.querySelectorAll<HTMLElement>(".react-pdf__Page"))
          .map((node, i) => {
            ...
            return { page: num, top: node.getBoundingClientRect().top - rootTop };
          })
      ...
      const step = (ts: number) => {
        ...
        if (dwellUntilRef.current && ts >= dwellUntilRef.current) dwellUntilRef.current = 0;
        ...
        if (max > 2) {
          ...
          if (routeMode) {
            ...
            // Repeat mode: pause when a matching page boundary crosses the top
            // edge of the viewport, then resume automatically after the dwell.
            if (ts - pageTopsAt > 500 || pageTops.length === 0) {
              pageTopsAt = ts;
              measurePages();
            }
            ...
            // Travelling up → park on the last boundary we passed.
            const crossed = crossedBoundary(pageTops, prevPos, posRef.current, dirRef.current, cfg);

            if (crossed && dwellPageRef.current !== crossed.page) {
    ...
    if (ifr) {
      ...
      const handler = (e: MessageEvent) => {
        ...
        const d = e.data as { type?: string; dir?: unknown; atEnd?: unknown };
        const cfgNow = dwellRef.current;
        const routeNow =
          cfgNow.enabled && cfgNow.parity === "route" && cfgNow.route.length > 0;
  ...
  }, [stop, deactivate, targetRef, iframeRef, pushDwellToIframe]);
