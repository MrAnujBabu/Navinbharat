...
const NATIVE_REMOTE_FETCH_TIMEOUT_MS = 180000;
...
export function useLocalPdfSource(url: string): LocalPdfState {
  ...
  useEffect(() => {
    ...
    // APK fix: even CORS-safe pdf-proxy streams can stall inside Android
    // WebView/pdf.js range loading (symptom: `drive • loading • pdf-proxy` then
    // blank/error). On native, fetch ALL remote PDF/proxy bytes through
    // CapacitorHttp and hand pdf.js a Uint8Array. Web keeps streaming normally.
    // Drive proxy now returns a clean full stream with Range disabled. Let
    // pdf.js consume that stream directly on native too; preloading the whole
    // Drive file through CapacitorHttp adds huge delay and can OOM high-MB PDFs.
    const shouldMaterialize = isLocal || isResolvableRemote || (isNativePlatform && isRemoteHttp && !isDrivePdfProxy);
