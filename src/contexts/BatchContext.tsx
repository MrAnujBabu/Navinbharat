  ...
  useEffect(() => {
    ...
    const fetchBatches = async () => {
      ...
      try {
        ...
        const enrolledBatchesRaw: Batch[] = (data || [])
          .map((e: any) => e.courses)
          .filter(Boolean)
          .filter((c: any) => {
        ...
        const enrolledBatches: Batch[] = await Promise.all(
          enrolledBatchesRaw.map(async (batch) => ({
            ...batch,
            image_url: (await resolveContentUrl(batch.image_url)) ?? batch.image_url,
          }))
        );
