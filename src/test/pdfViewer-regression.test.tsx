...
describe("pdfViewerUrl helpers", () => {
  ...
  it("routes Drive URLs through the PDF proxy viewer", () => {
    const { embedUrl, isDrive } = resolveEmbedUrl(
      "https://drive.google.com/file/d/abc123/view"
    );
    expect(embedUrl.startsWith("/pdfjs/web/viewer.html")).toBe(true);
    expect(decodeURIComponent(embedUrl)).toContain("/pdf-proxy?kind=drive&id=abc123");
    expect(isDrive).toBe(false);
  });
});
...
describe("PdfViewer height calculation", () => {
  it("uses the locked 176px reservation so no white strip appears at the bottom", () => {
    ...
    // jsdom normalises the calc() expression with parentheses around the
    // first term — match on the locked 176px reservation instead.
...
describe("AutoScrollFab visibility", () => {
  ...
  it("renders the FAB for Google Drive PDFs after proxying into the in-app reader", () => {
    render(
