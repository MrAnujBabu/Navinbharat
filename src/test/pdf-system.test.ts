...
describe("Suite 7: Mobile viewport PDF fit (regression for centre-zoom clipping)", () => {
  ...
  it("7.2 fills the full width edge-to-edge (no white side strips)", () => {
    expect(computeFitPageWidth(480)).toBe(480);
    expect(computeFitPageWidth(390)).toBe(390);
  });
  ...
  it("7.5 container width caps page width even if viewport is larger", () => {
    ...
    expect(computeFitPageWidth(1200, 500)).toBe(500);
  });

  it("7.6 landscape surface fills its own width (no white side strips)", () => {
    // Rotated reader: window still reports portrait 412x892, surface is 892.
    expect(computeFitPageWidth(412, 892, 892)).toBe(892);
  });

  it("7.7 CSS rotation only when the lock failed AND viewport is portrait", () => {
    expect(shouldCssRotate(true, 412, 892)).toBe(false); // OS lock worked
    expect(shouldCssRotate(false, 412, 892)).toBe(true); // still portrait
    expect(shouldCssRotate(false, 892, 412)).toBe(false); // already landscape
  });
});
