...
describe("deep links", () => {
  it("accepts the production app-link host", () => {
    expect(APP_LINK_HOSTS).toContain("naveenbharat.vercel.app");
    expect(toInternalPath("https://naveenbharat.vercel.app/course/12")).toBe("/course/12");
  });
  ...
  it("no longer claims the retired sadguru host", () => {
    expect(APP_LINK_HOSTS).not.toContain("sadguruclasses.vercel.app");
    expect(toInternalPath("https://sadguruclasses.vercel.app/course/12")).toBeNull();
  });

  it("rejects unclaimed paths on a trusted host", () => {
    expect(toInternalPath("https://naveenbharat.vercel.app/admin")).toBeNull();
  });
  ...
  it("preserves hash anchors", () => {
    expect(toInternalPath("https://naveenbharat.vercel.app/lesson/9#t=120")).toBe("/lesson/9#t=120");
  });
