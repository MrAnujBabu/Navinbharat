...
describe("deep links", () => {
  it("accepts the production app-link host", () => {
    expect(APP_LINK_HOSTS).toContain("navinbharat.vercel.app");
    expect(toInternalPath("https://navinbharat.vercel.app/course/12")).toBe("/course/12");
  });
  ...
  it("rejects unclaimed paths on a trusted host", () => {
    expect(toInternalPath("https://navinbharat.vercel.app/admin")).toBeNull();
  });
  ...
  it("preserves hash anchors", () => {
    expect(toInternalPath("https://navinbharat.vercel.app/lesson/9#t=120")).toBe("/lesson/9#t=120");
  });
