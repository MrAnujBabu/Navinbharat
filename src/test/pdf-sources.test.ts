...
describe("PDF source routing", () => {
  ...
  it("never treats signed Notion attachments or their spaceId as pages", () => {
    const attachment = "https://file.notion.so/f/f/1938ce59-04b0-81e2-8661-000355379849/lecture.pdf?spaceId=1938ce59-04b0-81e2-8661-000355379849&table=block&id=36a8ce59-04b0-81c3-928d-db1a952f2561";
    expect(isNotion(attachment)).toBe(false);
    expect(extractNotionPageId(attachment)).toBeNull();
  });

  it("extracts page ids only from a Notion page path or the p parameter", () => {
    const page = "https://sunset-waxflower-f5c.notion.site/Lecture-series-36a8ce5904b081c3928ddb1a952f2561?pvs=4";
    expect(extractNotionPageId(page)).toBe("36a8ce5904b081c3928ddb1a952f2561");
    expect(extractNotionPageId("https://www.notion.so?p=36a8ce5904b081c3928ddb1a952f2561&spaceId=1938ce5904b081e28661000355379849")).toBe("36a8ce5904b081c3928ddb1a952f2561");
  });
...
describe("Universal document link support (7 types)", () => {
  ...
  it("routes Vedantu notes storage through pdf-proxy + in-app pdf.js", () => {
    const vedantu = "https://prod-recordings.vedantu.com/NOTES/PROD/6a7eb202ce63b65a22dd7742.pdf";
    expect(isVedantuRecordings(vedantu)).toBe(true);
    expect(isVedantuRecordings("https://prod-recordings.vedantu.com.evil.tld/x.pdf")).toBe(false);
    expect(isKnownNonPdfWebUrl(vedantu)).toBe(false);
    expect(isLikelyPdfUrl(vedantu)).toBe(true);
    const rendered = renderablePdfUrl(vedantu);
    expect(rendered).toContain("pdf-proxy");
    expect(decodeURIComponent(rendered)).toContain("kind=url");
