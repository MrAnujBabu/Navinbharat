  ...
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        const openUrl = (App as unknown as { openUrl?: (options: { url: string }) => Promise<{ completed?: boolean }> }).openUrl;
        if (!openUrl) throw new Error("openUrl unavailable");
        const res = await openUrl({ url: uri });
