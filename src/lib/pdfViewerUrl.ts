...
/** Supabase project ref, used to locate the auth-token entry in localStorage. */
const SUPABASE_REF = (() => {
  try {
...
/**
 * Read the current Supabase access token synchronously from storage.
 * pdf-proxy is authenticated, but pdf.js loads the URL inside an
 * <iframe>/worker that can't set request headers — so the JWT rides in a
 * `?token=` query param instead. Returns null when signed out.
 */
const currentAccessToken = (): string | null => {
  try {
    if (typeof localStorage === "undefined" || !SUPABASE_REF) return null;
