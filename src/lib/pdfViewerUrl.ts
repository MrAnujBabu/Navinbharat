...
import { supabaseFunctionsUrl } from "@/integrations/supabase/runtime";
...
export const isNotion = (url: string): boolean => {
  ...
  try {
    ...
    // Notion attachments use signed `file.notion.so` URLs whose query string
    // contains a workspace `spaceId`. They are document bytes, not pages.
    // Treating them as pages makes extractNotionPageId pick the workspace id
    // and repeatedly call notion-page with an id that can never be published.
    if (
      host === "file.notion.so" ||
      host.startsWith("file.") && host.endsWith(".notion.so") ||
      host.startsWith("secure.") && host.endsWith(".notion.so")
    ) {
      return false;
    }
    return (
...
export const extractNotionPageId = (url: string): string | null => {
  ...
  // Never inspect signed attachment URLs. Their query parameters contain
  // unrelated UUIDs (spaceId, blockId) that are not fetchable page ids.
  if (!isNotion(url)) return null;

  try {
    const parsed = new URL(url, /^https?:\/\//i.test(url) ? undefined : "https://notion.so");
    const queryId = normalize(parsed.searchParams.get("p"));
    if (queryId) return queryId;
    const pathId = normalize(decodeURIComponent(parsed.pathname));
...
export const notionPageProxyUrl = (pageId: string): string =>
  `${functionUrl("notion-page")}?id=${encodeURIComponent(pageId)}`;
...
/**
 * Vedantu lecture-notes storage — e.g.
 * `https://prod-recordings.vedantu.com/NOTES/PROD/<id>.pdf`.
 *
 * The upstream is a valid, range-streamable `application/pdf` but sends NO
 * `access-control-allow-origin` header and `x-frame-options: SAMEORIGIN`, so
 * neither a direct pdf.js fetch nor an iframe can read it. Bytes must be
 * relayed through pdf-proxy.
 */
export const isVedantuRecordings = (url: string): boolean => {
  try {
    return new URL(url).hostname.toLowerCase() === "prod-recordings.vedantu.com";
  } catch {
    return /^https:\/\/prod-recordings\.vedantu\.com\//i.test(url);
  }
};
...
export const googleDrivePdfProxyUrl = (url: string): string | null => {
  ...
  if (!id) return null;
  return withAccessToken(`${functionUrl("pdf-proxy")}?kind=drive&id=${encodeURIComponent(id)}`);
};
...
export const remotePdfProxyUrl = (url: string): string =>
  withAccessToken(`${functionUrl("pdf-proxy")}?kind=url&url=${encodeURIComponent(url)}`);

/**
 * CORS-safe byte proxy for Office documents (docx/pptx/xlsx). Same allow-list
 * and enrollment gate as the PDF route, but without the `%PDF-` signature
 * check — these files are ZIP containers rendered client-side.
 */
export const officeProxyUrl = (url: string): string =>
  withAccessToken(`${functionUrl("pdf-proxy")}?kind=office&url=${encodeURIComponent(url)}`);

/** Office document detection from a URL or filename. */
export const officeDocKind = (probe: string): "docx" | "xlsx" | "pptx" | "legacy" | null => {
  const m = probe.toLowerCase().match(/\.(docx|xlsx|xlsm|csv|pptx|doc|xls|ppt)(?:[?#]|$)/);
  if (!m) return null;
  if (m[1] === "docx") return "docx";
  if (m[1] === "pptx") return "pptx";
  if (m[1] === "xlsx" || m[1] === "xlsm" || m[1] === "csv") return "xlsx";
  return "legacy";
};
...
export const archiveOrgPdfProxyUrl = (url: string): string | null => {
  ...
  return withAccessToken(
    `${functionUrl("pdf-proxy")}?kind=archive&id=${encodeURIComponent(id)}&url=${encodeURIComponent(url)}`,
  );
};
