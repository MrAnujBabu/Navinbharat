...
/** Convert Drive PDFs to a same-origin/CORS PDF source so autoscroll can work. */
export const renderablePdfUrl = (url: string): string => googleDrivePdfProxyUrl(url) || url;
...
export function resolveEmbedUrl(url: string): {
  ...
  if (isGoogleDrive(url)) {
    ...
    if (fileId) {
      const proxyUrl = googleDrivePdfProxyUrl(url);
      return {
        embedUrl: proxyUrl ? pdfJsViewerUrl(proxyUrl) : `https://drive.google.com/file/d/${fileId}/preview`,
