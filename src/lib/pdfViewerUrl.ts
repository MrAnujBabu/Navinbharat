...
export function resolveEmbedUrl(rawUrl: string): {
  ...
  // Google Drive → embed Drive's own /preview iframe on web (keeps the
  // pdf-proxy edge function cold). On native (Capacitor) WebView, Drive's
  // /preview iframe is blank-screen flaky (cross-origin + XFO/CSP), so we
  // route through pdf-proxy → self-hosted pdf.js viewer to render real bytes
  // inside the app.
