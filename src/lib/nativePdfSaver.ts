 ...
 *   killed the app), then writes to `Documents/Naveen Bharat/<filename>`
...
const APP_FOLDER = "Naveen Bharat";
...
export async function savePdfToDevice(
  ...
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      const savedUri = (result as { uri?: string }).uri || (await Filesystem.getUri({ path, directory: Directory.Documents })).uri;
