...
export function computeFitPageWidth(
  ...
  // When the reader surface is CSS-rotated into pseudo-landscape its layout
  // width is the viewport *height*. Clamping to the portrait viewport width
  // then letterboxed the page into the top-left corner. Allow the larger
  // screen dimension as the upper bound so rotated surfaces fill edge-to-edge.
  const limit = Math.max(vp, vh);
  const cw = containerWidth && containerWidth > 0 ? Math.floor(containerWidth) : vp;
  const bounded = Math.min(cw, limit);
