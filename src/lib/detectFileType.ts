...
export function isKnownNonPdfWebUrl(url: string): boolean {
  ...
  // Handles both normal URLs (`https://x.notion.site/...`) and scheme-less
  // values accidentally saved by the CMS (`x.notion.site/...`).
  ...
  try {
    ...
    // Fall back to substring matching for malformed-but-still-openable URLs.
