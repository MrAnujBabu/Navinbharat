...
export async function openPdfHybrid({ url, fileName, record }: HybridOpenInput): Promise<boolean> {
  ...
  if (/drive\.google\.com|docs\.google\.com|notion\.(site|so)/i.test(url)) {
    try {
      ...
      // Do NOT fall back to PdfViewer for Drive/Docs/Notion on native: those
      // providers commonly block iframe/embedded PDF surfaces and leave the
      // user on the exact blank white page reported in APK QA.
      return true;
    }
  }
