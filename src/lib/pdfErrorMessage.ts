...
export function friendlyPdfErrorMessage(err: unknown, src: string | null | undefined): string {
  ...
  if (/Storage proxy unavailable|Storage proxy HTTP|Unsupported Naveen Bharat storage link/i.test(msg)) {
    return "Document service didn't respond. Tap Retry; if it keeps failing, tell the admin.";
  }

  // ── Archive.org ────────────────────────────────────────────────────────
  // pdf.js surfaces our typed proxy errors as a bare "Unexpected server
  // response (502)". For archive-backed sources say what actually happened
  // and what the student can do about it.
  const isArchive = /archive\.org|[?&]kind=archive/i.test(src || "");
  if (isArchive) {
    if (/archive_unavailable/i.test(`${code} ${msg}`) || status === 502 || status === 503 || status === 504) {
      return "Archive.org abhi is item ke liye respond nahi kar raha — Retry tap karein, ya browser me kholein.";
    }
    if (status === 404 || /No PDF file found in this archive/i.test(msg)) {
      return "Is archive.org item me koi PDF nahi mili (link galat ya item hata diya gaya). Uploader se naya link maangein.";
    }
    if (status === 415 || /not_pdf/i.test(`${code} ${msg}`)) {
      return "Archive.org ne PDF ki jagah kuch aur bheja. Retry tap karein; phir bhi fail ho to link replace karwayein.";
    }
  }
  ...
  if (/InvalidPDFException|Invalid PDF structure|InvalidPdf|missing pdf signature/i.test(`${e.name || ""} ${msg}`)) {
    // Vedantu notes storage serves valid PDF bytes but no CORS header, so a
    // direct (unproxied) fetch surfaces as a bogus "Invalid PDF structure".
    if (/prod-recordings\.vedantu\.com/i.test(src || "")) {
      return "Vedantu notes link ko app ne seedha nahi padh saka (CORS blocked). Tap Retry — app isko proxy ke through load karega; phir bhi fail ho to browser me kholein.";
    }
