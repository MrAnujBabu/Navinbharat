 ...
 *
 * `resolveContentUrl` turns either form into a short-lived signed URL. The
 * bucket is now fully private, so BOTH public and gated folders are signed
 * (anon SELECT policies still let public-folder objects be signed without a
 * session). External URLs (Notion, Drive, external CDNs, or objects in a
...
export async function resolveContentUrl(
  ...
  // Bucket is private. Both public-folder and gated-folder objects need a
  // signed URL — the legacy `/object/public/content/…` endpoint returns 400
  // now that the bucket is private. Public folders still allow anon SELECT
  // via RLS, so createSignedUrl works for them without a session.
  void isPublicFolder; // retained for future per-folder TTL tuning
