...
//
// All reload decisions are delegated to `reloadArbiter`, which owns a single
// shared cooldown (max 1 auto-reload per 60s) and never reloads a hidden WebView.

import { redactUrl, addBreadcrumb } from "./sentry";

import { recordDiagnostic } from "./freezeDiagnostics";
import { requestReload } from "./reloadArbiter";
...
// by `reloadArbiter` so this system can no longer consume the guard that
// resume-recovery or the boot watchdog needs. See src/lib/reloadArbiter.ts.
function safeReload(reason: string) {
  const decision = requestReload({ system: "crash-shield", reason });
  if (decision !== "granted") {
    // Reset the rejection window so a single fresh rejection after a
    // suppressed/deferred decision doesn't re-trigger this path on every event.
    rejectionCount = 0;
...
/** Give known heavy UI work (PDF decode/fullscreen transition) a bounded grace period. */
export function suppressCrashShield(durationMs = 5_000): void {
  const safeDuration = Math.max(0, Math.min(durationMs, 30_000));
  heartbeatLastTick = Date.now();
  heartbeatSuppressUntil = Math.max(heartbeatSuppressUntil, Date.now() + safeDuration);
}
...
function startHeartbeat() {
  ...
  const tick = () => {
    ...
    if (gap > HEARTBEAT_FREEZE_THRESHOLD_MS) {
      recordDiagnostic("freeze", `main thread blocked ${gap}ms`);
