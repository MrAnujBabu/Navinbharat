...
async function fetchJsonViaNativeHttp<T>(url: string, headers: Record<string, string>, signal?: AbortSignal): Promise<T | null> {
  ...
  try {
    ...
    // CapacitorHttp raises "Failed to fetch" / "network error" for transient
    // Android network hiccups. Return null so callers fall back to the
    // browser `fetch()` path instead of raising an unhandled rejection.
    return null;
  }
}
...
export async function fetchNaveenStoragePdf(url: string, signal?: AbortSignal): Promise<Blob> {
  ...
  try {
    nativeBlob = await requestPdfViaNativeHttp(fileUrl, {
      method: "POST",
      signal,
      headers: {
    ...
    if ((err as { name?: string })?.name === "AbortError") throw err;
    // Fall through to browser fetch on native HTTP transient failures.
    nativeBlob = null;
  }
