...
export const openNativeRazorpayCheckout = async (
  ...
  let Checkout: { open: (payload: Record<string, unknown>) => Promise<unknown> } | undefined;
  try {
  ...
  try {
    ...
    const msg = getErrorMessage(e, "") || (e as { errorMessage?: string })?.errorMessage || String(e ?? "");
  ...
  let parsed: unknown = (result as { response?: unknown } | null)?.response ?? result;
  ...
  const success = parsed as Partial<RazorpaySuccessResponse> | null;
  ...
  return {
    ...
    razorpay_order_id: success.razorpay_order_id ?? options.order_id,
    razorpay_signature: success.razorpay_signature as string,
  };
};
