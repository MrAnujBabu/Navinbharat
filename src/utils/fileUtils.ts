...
export const downloadFile = async (
  ...
  const fetchBlob = async () => {
    ...
    if (!res.ok) {
      // Same reason as nativePdfHttp: without host + status the Sentry issue
      // "Download failed (HTTP 500)" points at no upstream at all.
      const err = new Error(`Download failed (HTTP ${res.status})`) as Error & {
        status?: number;
        host?: string;
        urlPrefix?: string;
      };
      err.status = res.status;
      try { err.host = new URL(downloadUrl, window.location.href).host; } catch { err.host = "unknown-host"; }
      err.urlPrefix = downloadUrl.slice(0, 160);
      throw err;
    }
  ...
  if (isNativePlatform()) {
    ...
    try {
      ...
      // Breadcrumb only. This function rethrows, and the caller reports the
      // *same* failure — logging it here as an error opened a second Sentry
      // issue ("Native blob fallback failed" + "Failed to fetch") for one
      // download. The breadcrumb keeps the context without the duplicate.
      addBreadcrumb("download", "[downloadFile] native blob fallback failed", {
        surface: "downloadFile",
