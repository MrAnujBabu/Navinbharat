...
export const APP_SCHEME = "com.naveenbharat.app";

/** Verified https hosts whose links open inside the app (Android App Links). */
export const APP_LINK_HOSTS = [
  // Legacy deployment name of this same project — kept so old links keep working.
  // NOTE: its assetlinks.json must list package_name "com.naveenbharat.app"
  // (currently serves the old "com.sadguru.classes" — see docs/DEEP-LINKS.md).
  "sadguruclasses.vercel.app",
  // Naveen Bharat (current brand) — mirrored in AndroidManifest App Links.
  "naveenbharat.vercel.app",
  // "naveenbharat.in" / "www.naveenbharat.in" removed: not wired as Vercel custom
  // domains (HTTP 000). Re-add here + AndroidManifest once the domains are configured.
] as const;
