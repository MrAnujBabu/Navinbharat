...
export default function NotionPageRenderer({ url, title, onClose, onReady }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(notionPageProxyUrl(pageId), { signal: controller.signal })
      .then(async (res) => {
      ...
      .then((data) => {
        if (cancelled) return;
        if (!data?.recordMap?.block) throw new Error("Empty Notion response");
        traceReader("notion", "ready", "notion-fetch-success", { blocks: Object.keys(data.recordMap.block).length });
        setRecordMap(data.recordMap);
      })
      .catch((e) => {
        if (cancelled) return;
        const msg = e?.name === "AbortError" ? "Notion page timed out." : e?.message || "Failed to load";
        traceReader("notion", "error", "notion-fetch-error", { message: msg });
        setError(msg);
