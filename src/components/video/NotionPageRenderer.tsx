...
import { extractNotionPageId, notionPageProxyUrl, officeDocKind } from "../../lib/pdfViewerUrl";
import { Button } from "@/components/ui/button";
import OfficeDocViewer from "../library/OfficeDocViewer";
...
// Database (table / board / gallery) rendering lives in a separate entry point.
// Without it react-notion-x falls back to an empty component for every
// `collection_view` block, which is why database-driven Notion pages
// (e.g. the Zoology index) rendered as a blank scroll area.
const NotionCollection = lazyWithRetry(() =>
...
// Note: this react-notion-x build ships collection styles inside the base
// styles.css above — there is no separate third-party/collection.css.
...
class NotionPageRequestError extends Error {
  constructor(message: string, readonly code?: string) {
    super(message);
    this.name = "NotionPageRequestError";
  }
}

const isExpectedNotionAccessError = (error: unknown): boolean => {
  const code = error instanceof NotionPageRequestError ? error.code : undefined;
  return code === "notion_not_public" || code === "notion_not_found";
};
...
/**
 * Minimal structural types for the Notion `recordMap` payload. react-notion-x
 * ships its own `ExtendedRecordMap`, but the edge function returns a superset
 * with permission stubs (`{ role }` with no `value`), so we model only what we
 * actually read here and cast once at the render boundary.
 */
type NotionBlockValue = {
  type?: string;
  properties?: { title?: unknown[][] };
};
type NotionBlockEntry = { role?: string; value?: NotionBlockValue | { value?: NotionBlockValue } };
type NotionRecordMapLike = { block?: Record<string, NotionBlockEntry | undefined> };

/** Unwrap the `{ value: { value } }` double-nesting Notion sometimes returns. */
const notionBlockValue = (entry: NotionBlockEntry | undefined): NotionBlockValue | undefined => {
  const raw = entry?.value as NotionBlockValue & { value?: NotionBlockValue } | undefined;
  return raw?.value ?? raw;
};

/** Blocks that count as "the page has its own content worth reading". */
function textBlockCount(recordMap: NotionRecordMapLike | null | undefined): number {
  let n = 0;
  for (const entry of Object.values(recordMap?.block ?? {})) {
    const value = notionBlockValue(entry);
...
/**
 * Notion returns permission stubs (`{ role: "editor" }` with no `value`) for
 * blocks a public share does not expose — on a large database page that is a
 * third of the payload. react-notion-x walks them and paints empty rows, so
 * strip them before rendering.
 */
function pruneEmptyBlocks<T extends NotionRecordMapLike>(recordMap: T): T {
  const src = recordMap?.block ?? {};
  const block: Record<string, NotionBlockEntry | undefined> = {};
  let dropped = 0;
  for (const [id, entry] of Object.entries(src)) {
    const value = notionBlockValue(entry);
    if (!value || !value.type) { dropped += 1; continue; }
    block[id] = entry;
  }
  return dropped === 0 ? recordMap : { ...recordMap, block };
}
...
export default function NotionPageRenderer({ url, title, onClose, onReady, onDocument }: Props) {
  ...
  const [officeDoc, setOfficeDoc] = useState<{ id: string; name: string; url: string } | null>(null);
  ...
  const [recordMap, setRecordMap] = useState<NotionRecordMapLike | null>(null);
  ...
  const exportToPdf = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        toast({ title: "Saved", description: "PDF saved to Documents/Naveen Bharat." });
      } finally {
        if (revokeTimerRef.current) clearTimeout(revokeTimerRef.current);
  ...
  useEffect(() => {
    ...
    const fetchOnce = async (): Promise<unknown> => {
      ...
      if (!res.ok) {
        // Surface the proxy's own message (e.g. "not shared publicly") instead
        // of a bare `HTTP 404`, which rendered as a blank screen.
        let detail = "";
        let code = "";
        try {
        ...
        throw new NotionPageRequestError(detail || `HTTP ${res.status}`, code || undefined);
      }
      return res.json();
    };
    ...
    (async () => {
      ...
      try {
        ...
        if (TRANSIENT.test(msg)) {
          ...
          try {
            ...
            setError(msg2);
            if (!isExpectedNotionAccessError(e2)) {
          ...
          setError(friendly);
          if (!isExpectedNotionAccessError(e)) {
      ...
      setRecordMap(pruneEmptyBlocks(recordMap as NotionRecordMapLike));
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- fetch is keyed on the active pageId; `url` only seeds the stack and must not refetch.
  }, [pageId]);
  ...
  const isOffice = (doc: { name: string; url: string }) =>
    officeDocKind(`${doc.name} ${doc.url}`) !== null;
  useEffect(() => {
    if (!onDocument || !soleDocument || !recordMap) return;
    // Office attachments render in OfficeDocViewer, not the PDF reader.
    if (isOffice(soleDocument)) return;
  ...
  if (error) {
    // A failing *subpage* (e.g. a database row that was never published to the
    // web) must not blank out the whole reader: pop back to the parent page and
    // tell the user why, keeping the already-loaded content on screen.
    if (stack.length > 1) {
      return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 p-6 text-center">
          <p className="max-w-sm text-sm text-muted-foreground">{error}</p>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" onClick={popOrClose}>Back</Button>
            <Button
              variant="outline"
              size="sm"
  ...
  return (
    ...
    >
        ...
        <div className="notion-export-ignore px-4 pt-16">
          {documents.map((doc) => (
            <button
              key={doc.id}
              type="button"
              onClick={() => (isOffice(doc) ? setOfficeDoc(doc) : onDocument?.(doc.url))}
              disabled={!isOffice(doc) && !onDocument}
      ...
      {/* Word / Excel / PowerPoint attachment opened in the in-app renderer. */}
      {officeDoc && (
        <div className="fixed inset-0 z-[60] flex flex-col bg-background">
          <div className="safe-area-top flex min-h-[48px] items-center gap-2 border-b bg-card/95 px-3">
            <button
              type="button"
            ...
            <span className="min-w-0 flex-1 truncate text-sm font-semibold">{officeDoc.name || "Document"}</span>
          </div>
          <div className="min-h-0 flex-1">
            <OfficeDocViewer url={officeDoc.url} filename={officeDoc.name} title={officeDoc.name} />
          </div>
        </div>
      )}
      ...
      {/* Floating "Download as PDF" — bottom-right, safe-area aware. Exports
          ...
          savePdfToDevice → Capacitor Filesystem (Documents/Naveen Bharat) on
      ...
      <Suspense fallback={<div className="p-4 text-sm text-muted-foreground">Rendering…</div>}>
        <NotionRenderer
          recordMap={recordMap as unknown as Parameters<typeof NotionRenderer>[0]["recordMap"]}
          fullPage={false}
          darkMode={false}
          disableHeader
          ...
          components={{
            // Databases (table / board / gallery views).
            ...
            PageLink: ({ href, children, ...rest }: { href?: string; children?: React.ReactNode } & Record<string, unknown>) => (
              <a
                {...rest}
                href={href}
                onClick={(e) => {
                  ...
                  if (typeof href === "string" && href) {
                    ...
                    // Only navigate in-app when the target block actually exists
                    // in the current recordMap. Collection rows / space-level
                    // stubs point at ids that Notion never published, and
                    // fetching those 404s the edge function and blanks the view.
                    const id = extractNotionPageId(next); // 32-char, no dashes
                    const dashed = id
                      ? `${id.slice(0, 8)}-${id.slice(8, 12)}-${id.slice(12, 16)}-${id.slice(16, 20)}-${id.slice(20)}`
                      : null;
                    const known = !!(
                      id && (notionBlockValue(recordMap?.block?.[id]) || (dashed && notionBlockValue(recordMap?.block?.[dashed])))
                    );
                    if (!known) {
