...
export default function NotionPageRenderer({ url, title }: Props) {
  ...
  return (
    <div className="relative h-full w-full overflow-auto bg-background notion-app-wrapper">
      {/* Floating back/close FAB — pops subpage stack, else exits reader.
          Positioned above safe-area so it never overlaps the iOS home indicator. */}
      <button
        type="button"
        onClick={popOrClose}
        aria-label={canGoBack ? "Back to previous page" : "Close Notion preview"}
        className="fixed right-4 z-50 inline-flex h-11 w-11 items-center justify-center rounded-full bg-foreground/90 text-background shadow-lg ring-1 ring-border backdrop-blur transition-transform active:scale-95"
