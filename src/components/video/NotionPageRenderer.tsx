  ...
  return (
    <div className="relative h-full w-full overflow-auto bg-background notion-app-wrapper">
      ...
      <button
        ...
        className="fixed left-3 z-50 inline-flex h-9 w-9 items-center justify-center rounded-full bg-background/80 text-foreground shadow-md ring-1 ring-border backdrop-blur transition-transform active:scale-95"
        style={{ top: "calc(env(safe-area-inset-top, 0px) + 10px)" }}
      >
        <ArrowLeft className="h-4 w-4" />
      </button>

      {/* Floating Exit FAB — bottom-left, mirror of the Download FAB. Acts
          as a hardware-back equivalent: pops the in-page subpage stack if
          one exists, otherwise closes the entire Notion preview. Larger and
          more visible than the top-left arrow for thumb reach. */}
      <button
        type="button"
        onClick={popOrClose}
        aria-label={canGoBack ? "Back" : "Exit Notion page"}
        className="fixed left-4 z-50 inline-flex h-12 w-12 items-center justify-center rounded-full bg-foreground/85 text-background shadow-lg ring-1 ring-border transition-transform active:scale-95"
      ...
      <button
        ...
        className="fixed right-4 z-50 inline-flex h-12 w-12 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg ring-1 ring-border transition-transform active:scale-95 disabled:opacity-60"
        style={{ bottom: "calc(env(safe-area-inset-bottom, 0px) + 18px)" }}
      >
