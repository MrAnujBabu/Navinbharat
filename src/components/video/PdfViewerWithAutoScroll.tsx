...
import { isGoogleDocs } from "../../lib/pdfViewerUrl";
...
export default function PdfViewerWithAutoScroll({
  ...
  return (
    <div className={chromeVisible ? "relative h-full w-full" : "absolute inset-0"}>
      ...
      />
      {/* Google Drive PDFs are proxied into FastPdfReader now, so autoscroll is
          enabled there. Google Docs is still cross-origin/non-PDF, so hide it. */}
      {!isGoogleDocs(url) && (
        <div
