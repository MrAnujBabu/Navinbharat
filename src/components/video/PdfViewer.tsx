...
const PdfViewerInner = forwardRef<PdfViewerHandle, PdfViewerProps>(
  ({ url: rawUrl, title, filename, chromeVisible = true, onSurfaceTap, onFirstByte, initialPage, onPageChange }, ref) => {
    ...
    useEffect(() => {
      ...
      // Drive/Docs debug breadcrumb — surfaces in chrome://inspect so we can
      // verify Drive PDFs render via the in-app /preview iframe (and never
      // get punted to a native viewer).
      try {
