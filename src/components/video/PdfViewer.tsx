  ...
  ({ url: rawUrl, title, filename, chromeVisible = true, onSurfaceTap, onFirstByte, initialPage, onPageChange, onReady, onZoomChange, showLoadingOverlay = true, readerId }, ref) => {
    ...
    useEffect(() => {
      ...
      void supabase.auth.getSession().then(() => {
        if (alive) setAuthVersion((value) => value + 1);
      });
    ...
    const driveProxyUrl = useMemo(
      ...
      // eslint-disable-next-line react-hooks/exhaustive-deps -- authVersion is an intentional cache buster so a token refresh re-signs the URL.
      [driveId, url, authVersion]
    );
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- authVersion is an intentional cache buster so a token refresh re-signs the URL.
    const resolved = useMemo(() => resolveEmbedUrl(effectiveUrl), [effectiveUrl, authVersion]);
    ...
    useEffect(() => {
      ...
      try {
        if (driveProxyUrl) emitPdfLifecycle("pdf-proxy", readerId, { route: "drive", url: driveProxyUrl.slice(0, 160) });
      } catch {}
    }, [driveProxyUrl, effectiveUrl, isMd, readerId, useIframe, url]);
    ...
    useEffect(() => {
      ...
      const onPdfError = (event: Event) => {
        if (!pdfLifecycleMatches(event, readerId)) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- effectiveUrl is derived from the deps already listed; adding it would double-load the document.
    ...
    useEffect(() => {
      ...
      const onMessage = (event: MessageEvent) => {
        ...
        if (payload.type === "nb-pdf-progress") {
          ...
          emitPdfLifecycle("pdf-progress", readerId, { percent: payload.percent });
          return;
        }
        if (payload.type === "nb-pdf-pagesloaded") {
          ...
          emitPdfLifecycle("pdf-first-byte", readerId, { source: payload.type });
          return;
        }
        if (payload.type === "nb-pdf-ready" || payload.type === "nb-pdf-pagerendered") {
          ...
          emitPdfLifecycle("pdf-ready", readerId, { source: payload.type });
          onReady?.();
          return;
        }
        if (payload.type === "nb-pdf-error" || payload.type === "nb-pdf-timeout") {
          ...
          setShowOpenExternal(true);
          emitPdfLifecycle("pdf-error", readerId, { message: payload.message || "PDF iframe failed to render." });
        }
      };
      window.addEventListener("message", onMessage);
      return () => window.removeEventListener("message", onMessage);
    }, [isDrive, onReady, readerId, useIframe]);
      ...
      <div className={wrapperClass} style={wrapperStyle} onClick={onSurfaceTap}>
        <iframe
          ...
          onLoad={() => {
            ...
            emitPdfLifecycle("pdf-first-byte", readerId, { iframe: true });
            if (isDrive || isGoogleDocs(effectiveUrl) || isHtmlViewer) {
              emitPdfLifecycle("pdf-ready", readerId);
            }
            onReady?.();
          }}
        />
