...
const FastPdfReader = forwardRef<FastPdfReaderHandle, Props>(
  ({ url, title, showLoadingOverlay = true, onSurfaceTap, onFirstByte, initialPage, onPageChange, onReady }, ref) => {
    ...
    const scheduleTransportRetry = useCallback((message: string): boolean => {
      ...
      try {
        window.dispatchEvent(new CustomEvent("pdf-progress", {
          detail: { percent: -1, phase: "retrying", measured: false },
        }));
      } catch { /* noop */ }
      if (transportRetryTimerRef.current) window.clearTimeout(transportRetryTimerRef.current);
    ...
    const pixelRatio = useMemo(() => {
      ...
      return Math.max(1, Math.min(dpr, 2 / Math.max(1, zoom)));
    }, [zoom]);
    const releaseDistantPages = isArchiveSource(src) || zoom > 1.5;
    ...
    }, [data, fallbackData, fetchPdfBlobWithRetry, friendlyPdfError, route, scheduleTransportRetry, src, url]);
    ...
    const onLoadError = useCallback(
      ...
      [fetchWholeFileFallback, friendlyPdfError, route, scheduleTransportRetry, src, url]
    );
