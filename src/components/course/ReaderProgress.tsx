 ...
 */
const ReaderProgress = memo(({ visible, title, docKey, variant = "pdf" }: Props) => {
  ...
  const phaseLabel = phase === "connecting"
    ...
    : phase === "indexing"
      ...
      : phase === "rendering"
        ...
        // "Reconnecting" is only truthful when bytes were already measured and
        // the stream then went unmeasurable (a retry). A first load with no
        // Content-Length is still just "Opening …".
