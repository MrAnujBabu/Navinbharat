...
const DocumentReader = memo(
  ({ title, subtitle, badge, url, onBack, lessonId }: DocumentReaderProps) => {
    ...
    // (which navigates back to the previous course/lesson screen instead
    // of exiting the app or skipping straight to dashboard).
    useEffect(() => {
      try {
      ...
      return () => {
        ...
        // Clean up our sentinel if the reader is unmounted by an in-app
        // action (header back / fullscreen exit) rather than a hardware-back.
        try {
