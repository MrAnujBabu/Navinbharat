...
import { openNativeDocument } from "../../lib/native/openNativeDocument";
...
const DocumentReader = memo(
  ({ title, subtitle, badge, url, onBack, lessonId }: DocumentReaderProps) => {
    ...
    const [nativeOpenError, setNativeOpenError] = useState<string | null>(null);
    ...
    // APK-only: DPP PDFs are the highest-risk document flow because many links
    // are Drive/Notion/custom viewer pages. Android WebView can render those as
    // a blank white iframe, so DPPs and redirect-heavy URLs open through native
    // document/browser surfaces first. Web preview keeps the in-app pdf.js path.
    const badgeLabel = (badge || "").toUpperCase();
    const isDppDocument = badgeLabel === "DPP" || badgeLabel === "DPP_ATTEMPT";
    const shouldUseNativeBrowserSurface = isNative() && (isDppDocument || isRedirectHeavyUrl(url));
    ...
    useEffect(() => {
      ...
      setErrorMsg(null);
      setNativeOpenError(null);
      void openNativeDocument(url, { filename: title, preferBrowser: isRedirectHeavyUrl(url) }).then((opened) => {
        if (!opened) setNativeOpenError("Native PDF opener unavailable on this device.");
      }).catch((err) => {
        console.error("[DocumentReader] native browser open failed", err);
        setNativeOpenError("Could not open this document.");
      });
    }, [shouldUseNativeBrowserSurface, title, url]);
      ...
      return (
        <div className="fixed inset-0 z-50 flex flex-col bg-background">
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            {nativeOpenError ? (
              <p className="text-sm text-destructive">{nativeOpenError}</p>
            ) : (
              <>
