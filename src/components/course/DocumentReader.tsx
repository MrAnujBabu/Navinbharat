...
const DocumentReader = memo(
  ({ title, subtitle, badge, url, onBack, lessonId }: DocumentReaderProps) => {
    ...
    }, [markFirstByte, markPdfProgress, updateHealth]);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div className="absolute bottom-3 right-3 z-30 max-w-[72vw] rounded-full border border-border bg-background/90 px-3 py-1 text-[11px] font-medium text-muted-foreground shadow-sm backdrop-blur-sm">
            <span className="text-foreground">{health.route}</span>
            <span className="mx-1">•</span>
            <span>{health.state}</span>
            <span className="mx-1">•</span>
            <span className="inline-block max-w-[34vw] truncate align-bottom">{health.lastEvent}</span>
          </div>
