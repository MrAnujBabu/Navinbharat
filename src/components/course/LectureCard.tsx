...
import notesThumb from "../../assets/thumbnails/notes-default.svg";
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          {/* PDF / Notion thumbnail tile — uses the shared notes-default SVG
              so the card reads as a real document preview, not a blank
              colour swatch. Notion links get a distinct label below. */}
          <div className={cn(
            "min-w-[56px] w-[56px] h-[56px] rounded-xl overflow-hidden flex items-center justify-center flex-shrink-0 border border-border/40",
            isCompleted ? "bg-green-50 dark:bg-green-950/30" : "bg-white dark:bg-zinc-900"
          )}>
            {isLocked ? (
              ...
              <img src={notesThumb} alt="" loading="lazy" decoding="async" className="w-full h-full object-cover" />
            )}
          </div>
          ...
          {(() => {
            ...
            const kindClass = isNotionLink ? "text-rose-600 dark:text-rose-400" : "text-purple-600 dark:text-purple-400";
            return (
          ...
          {/* Open chevron CTA */}
          <div className="shrink-0 inline-flex items-center justify-center bg-zinc-900 group-hover:bg-zinc-800 text-white dark:bg-zinc-100 dark:text-zinc-900 dark:group-hover:bg-white rounded-xl h-10 px-3 text-xs font-semibold transition-colors">
            Open
          </div>
        </div>
      </div>
