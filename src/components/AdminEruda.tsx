    ...
    import("eruda")
      .then(({ default: eruda }) => {
        try {
          ...
          // the admin console. See main.tsx early-boot path for context.
          void import("@/lib/nativeDebug").then((mod) => {
            try {
              const filter = mod.isExpectedConsoleNoise;
              if (!filter) return;
              const w = window as unknown as { __nb_eruda_filter_installed?: boolean };
              if (w.__nb_eruda_filter_installed) return;
