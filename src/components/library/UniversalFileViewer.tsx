...
function classify(fileType: string, url: string, filename?: string): Kind {
  ...
  if (["DOC", "DOCX", "PPT", "PPTX", "XLSX", "XLS", "XLSM", "CSV"].includes(t)) return "OFFICE";
  if (t === "IMAGE") return "IMAGE";
  if (t === "VIDEO") return "VIDEO";
  // URL-based inference
  const u = url.toLowerCase();
  if (/\.(pdf)(\?|#|$)/.test(u)) return "PDF";
  // Keep this in sync with `officeDocKind` in src/lib/pdfViewerUrl.ts —
  // xlsm/csv are rendered by OfficeDocViewer (exceljs) but used to fall
  // through to LINK and bounce out to the OS browser.
  if (/\.(doc|docx|ppt|pptx|xls|xlsx|xlsm|csv)(\?|#|$)/.test(u)) return "OFFICE";
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      ...
      <div className="relative flex-1 bg-neutral-100 dark:bg-neutral-900">
        {kind === "OFFICE" && (
          // Rendered in-app (mammoth / exceljs / pptx-preview) instead of the
          // Microsoft web viewer, which needs a publicly reachable URL and is
          // blocked in the APK WebView.
          <OfficeDocViewer url={url} filename={filename} title={title} />
        )}
        {kind === "IMAGE" && (
