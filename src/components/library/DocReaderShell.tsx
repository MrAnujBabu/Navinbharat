...
import { ArrowLeft, BookMarked, BookOpen, Download, Loader2, Maximize, Minimize, NotebookPen, X } from "lucide-react";
...
export default function DocReaderShell({
  ...
  // ── Browser fullscreen (F11-like) ───────────────────────────────────────
  // Own implementation instead of useFakeFullscreen: this shell already owns a
  // history sentinel + popstate handler, and a second pushState would make the
  // hardware back button close the reader instead of leaving fullscreen.
  const toggleFullscreen = useCallback(() => {
    ...
    try {
      ...
      // Fire without await so the user-gesture context survives (Android WebView).
      const req = el.requestFullscreen || el.webkitRequestFullscreen || el.mozRequestFullScreen;
      const r = req?.call(el) as Promise<void> | undefined;
      r?.catch?.(() => {
        // Native app / unsupported browser — immersive mode is already on.
        toast.message("Fullscreen isn't available here");
      });
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Never leave the browser stuck in fullscreen after the reader closes.
      try {
  ...
  return (
    <div ref={shellRef} className="fixed inset-0 z-[60] flex bg-background motion-safe:animate-in motion-safe:fade-in-0 motion-safe:duration-150" data-testid="doc-reader-shell">
      {/* Center column */}
      <div className="relative flex min-w-0 flex-1 flex-col" onClick={handleSurfaceTap}>
        ...
        <div
          ...
          style={
            pseudoLandscape
              ? {
                  // Browser refused the orientation lock — rotate the surface
                  // ourselves so the button still gives a landscape page.
                  top: 0,
                  left: 0,
                  right: "auto",
                  bottom: "auto",
