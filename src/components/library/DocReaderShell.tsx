  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-background">
      ...
      <div className="relative flex min-w-0 flex-1 flex-col" onClick={handleSurfaceTap}>
        ...
        {/* PDF surface — offset by safe-area-inset-top + header height so the
            first page is never hidden behind the status-bar notch or the
            floating header. (Previously top:0 caused the top ~44–60 px of
            CDN/jsDelivr-delivered PDFs to be cropped under the notch.)
            Audit fix (Batch): when the header auto-hides on scroll we
            collapse the top offset to just the safe-area inset — otherwise
            a ~48 px empty white strip stayed visible at the top of the
            reader (see screenshot from 20260703). */}
        <div
