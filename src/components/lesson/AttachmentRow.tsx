...
import { FileText, FileType2, Image as ImageIcon, Music, Video, File, Download, Loader2, ArrowDownToLine } from "lucide-react";
...
export function AttachmentRow({ attachment, onOpenPdf, resolveUrl, onDownloaded, className }: AttachmentRowProps) {
  ...
  return (
    ...
    >
      ...
      {/* Floating circular download button — always available, including for PDFs */}
      <button
        type="button"
        onClick={handleDownloadClick}
        disabled={downloading}
        aria-label={`Download ${attachment.title || attachment.file_name}`}
        className={cn(
          "flex-shrink-0 h-9 w-9 rounded-full flex items-center justify-center",
          "bg-primary text-primary-foreground shadow-md shadow-primary/20",
          "transition-all active:scale-95 hover:shadow-lg hover:shadow-primary/30",
          "disabled:opacity-70"
        )}
      >
        {downloading ? (
