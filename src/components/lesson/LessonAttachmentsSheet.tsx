...
export function LessonAttachmentsSheet({ open, onOpenChange, lessonId, lessonTitle, courseId }: Props) {
  ...
  const handleOpenPdf = useCallback(async (url: string, fileName: string, note?: LessonNote) => {
    if (!url) return;
    // Notes / DPP / PDF chips should open exactly like the lecture attachment
    // reader: inside LessonView's inline PdfViewer below the player. Avoid the
    // older full-screen DocReaderShell path because it creates a second reader
    // surface in the Capacitor APK and is the route that kept spinning for
    // Notion / Drive / CDN links.
    if (lessonId && courseId) {
      ...
      onOpenChange(false);
      navigate(`/classes/${encodeURIComponent(String(courseId))}/lessons?${params.toString()}`);
      return;
    }

    // Safety fallback for any future caller that does not know the course id.
    let resolved = url;
  ...
  }, [courseId, getResolvedUrl, lessonId, lessonTitle, navigate, onOpenChange]);
