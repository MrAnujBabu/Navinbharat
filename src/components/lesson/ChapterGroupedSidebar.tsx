...
interface ChapterGroupedSidebarProps {
  chapterMap: Map<
    string,
    { chapter: { id: string; code: string; title: string } | null; lessons: LessonRow[] }
  >;
  uncategorized: LessonRow[];
  lessons: LessonRow[];
  renderLesson: (lesson: LessonRow, globalIndex: number) => React.ReactNode;
...
export function ChapterGroupedSidebar({
  ...
  lessons.forEach((l: LessonRow, i: number) => globalIndexMap.set(String(l.id), i));
