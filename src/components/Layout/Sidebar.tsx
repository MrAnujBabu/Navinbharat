...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 h-full w-72 bg-sidebar z-[90] shadow-xl transition-transform duration-300 flex flex-col pl-safe-l",
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 pt-safe-t border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            <img src={logo} alt="Logo" width={40} height={40} loading="eager" decoding="async" className="h-10 w-10 rounded-lg" />
          ...
          <Button
            ...
            className="text-sidebar-foreground hover:bg-sidebar-accent active:scale-[0.94] transition-transform duration-150 ease-out"
          >
            <X className="h-5 w-5" />
          </Button>
        </div>
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => { void selectionHaptic(); onClose(); }}
                className={cn(
                  "flex items-center gap-3 px-4 py-3 mx-2 rounded-xl transition-all duration-150 ease-out active:scale-[0.99]",
                  active
          ...
          {isAuthenticated && isAdmin && (
            <>
              <Link
                ...
                onClick={() => { void selectionHaptic(); onClose(); }}
                className={cn(
                  "flex items-center gap-3 px-4 py-3 mx-2 mt-2 rounded-xl transition-all duration-150 ease-out active:scale-[0.99]",
              ...
              <Link
                ...
                onClick={() => { void selectionHaptic(); onClose(); }}
                className={cn(
                  "flex items-center gap-3 px-4 py-3 mx-2 mt-1 rounded-xl transition-all duration-150 ease-out active:scale-[0.99]",
        ...
        <div className="p-4 pb-safe-b border-t border-sidebar-border">
          ...
          {isAuthenticated ? (
            <Button
              ...
              className="w-full justify-start gap-3 text-sidebar-foreground/80 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground active:scale-[0.98] transition-all duration-150 ease-out"
            >
