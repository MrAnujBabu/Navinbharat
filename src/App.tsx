import { memo, useState, useEffect, Suspense, forwardRef } from "react";
...
const AdminPaymentsHealth = lazyWithRetry(() => import("./pages/AdminPaymentsHealth"));
const AdminErrors = lazyWithRetry(() => import("./pages/AdminErrors"));
const AdminPerformance = lazyWithRetry(() => import("./pages/AdminPerformance"));
const AdminBandwidth = lazyWithRetry(() => import("./pages/AdminBandwidth"));
const DebugDiagnostics = lazyWithRetry(() => import("./pages/DebugDiagnostics"));
...
const PublicRoute = forwardRef<unknown, { element: React.ReactElement }>(({ element }, _ref) => {
  ...
  // auth bootstrap unmounts the sign-in form, so anything typed in the first few
  // hundred ms is discarded and submit fails with "Please fill in all fields"
...
// forwardRef for the same reason as PublicRoute: react-router hands guards a ref
// and a plain function component logs a console.error that Sentry ingests.
const ProtectedRoute = forwardRef<unknown, { element: React.ReactElement }>(({ element }, _ref) => {
  const { isAuthenticated, isLoading } = useAuth();
...
const AdminRoute = forwardRef<unknown, { element: React.ReactElement }>(({ element }, _ref) => {
              ...
              <BrowserRouter>
                <NavigationHistoryProvider>
                  ...
                  <Routes>
                    ...
                    <Route path="/neet-class-11" element={<ExamLanding />} />
                    <Route path="/neet-class-12" element={<ExamLanding />} />
                    <Route path="/neet-dropper" element={<ExamLanding />} />
                    ...
                    <Route path="/admin/payments-health" element={<AdminRoute element={<AdminPaymentsHealth />} />} />
                    <Route path="/admin/errors" element={<AdminRoute element={<AdminErrors />} />} />
                    <Route path="/admin/performance" element={<AdminRoute element={<AdminPerformance />} />} />
                    <Route path="/admin/bandwidth" element={<AdminRoute element={<AdminBandwidth />} />} />
                    <Route path="/debug/diagnostics" element={<DebugDiagnostics />} />
