...
import type { LessonRow } from "@/types/rows";
    ...
    <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => onWatch(lesson)}>
      <CardHeader className="p-0">
        <div className="aspect-video bg-muted rounded-t-lg relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-overlay/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
            {lesson.isLocked ? (
              <Lock className="h-12 w-12 text-video-scrim-foreground" />
            ) : (
              <Play className="h-12 w-12 text-video-scrim-foreground" />
            )}
          </div>
...
const CoursePage = () => {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- `lessons` is a cheap fallback expression; hoisting it into its own memo adds a hook for no measurable win.
