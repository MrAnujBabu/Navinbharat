...
const CoursePage = () => {
  ...
  // Single RPC replaces 2 round-trips (course + lessons).
  // get_course_bundle enforces enrolment-gated privileged fields server-side.
  const bundleQuery = useQuery({
    queryKey: ['course-bundle', id],
    enabled: !!id && !!user,
    staleTime: 2 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_course_bundle', {
        _course_id: Number(id),
      });
  ...
  const course = bundleQuery.data?.course ?? null;
  const lessons = useMemo<Lesson[]>(
    () =>
      (bundleQuery.data?.lessons ?? []).map((l) => ({
        id: l.id,
        title: l.title,
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* SA fix: wrap Breadcrumbs so we can inset it below the status bar
          (Capacitor overlaysWebView=true means content flows behind the notch). */}
      <div className="sticky top-0 z-20" style={{ top: "env(safe-area-inset-top, 0px)" }}>
