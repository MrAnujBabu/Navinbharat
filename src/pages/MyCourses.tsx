...
import type { EnrollmentRow, LessonRow, ProgressRow } from "@/types/rows";
...
const formatShortDate = (dateString: string) => {
  ...
  return {
    // Show the year whenever it isn't the current one — "Start 2 Jun / End 2 Jun"
    // with no year read like a data bug on the course cards.
...
const MyCourses = () => {
  ...
  const fetchEnrolledCourses = useCallback(async () => {
    ...
    try {
      ...
      const enrolledCoursesRaw = (enrollments || []).map((enrollment: EnrollmentRow) => {
        const course = enrollment.courses;
        if (!course) return null;

        const courseLessons = allLessons.filter((l: LessonRow) => l.course_id === course.id);
        const courseLessonIds = new Set(courseLessons.map((l: LessonRow) => l.id));
        // Only count lessons that still belong to this course, de-duplicated by
        // lesson_id. Counting raw rows (or trusting p.course_id) let orphaned
        // progress rows from deleted/moved lessons push this above 100%.
        const completedLessonIds = new Set<string>();
        progressData.forEach((p: ProgressRow) => {
          if (p.completed && courseLessonIds.has(p.lesson_id)) {
