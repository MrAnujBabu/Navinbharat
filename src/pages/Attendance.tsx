...
import type { AttendanceRow, ProfileRow } from "@/types/rows";
...
const Attendance = () => {
  ...
  useEffect(() => {
    ...
    fetchStudents();
  // eslint-disable-next-line react-hooks/exhaustive-deps -- fetchStudents is a mount-scoped loader; adding it would refetch on every render.
  }, [selectedCourseId]);
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (!error && data) {
        const userIds = Array.from(new Set(data.map((e: AttendanceRow) => e.user_id).filter(Boolean)));
        ...
        if (userIds.length) {
          const { data: profs } = await supabase
            ...
            .in("id", userIds);
          (profs || []).forEach((p: ProfileRow) => profileMap.set(p.id, p));
        }
        setStudentList(data.map((e: AttendanceRow, idx: number) => {
          ...
          {loading ? (
            ...
            studentList.map((student) => (
              <StudentAttendanceRow
                key={student.id}
                student={{ ...student, id: Number(student.id) }}
