...
const Attendance = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Use the existing attendance table (student_id is bigint).
      // We store a hash/index for now since the table expects bigint student_id.
      // For proper integration the table should be migrated to uuid, but for backwards compat
      // we insert using a numeric mapping.
      const records = studentList.map((student, idx) => ({
        student_id: idx + 1, // numeric placeholder since table expects bigint
        date: dateStr,
