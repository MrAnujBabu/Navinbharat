...
import { openNativeRazorpayCheckout, RazorpayCancelledError } from "../utils/razorpayNative";
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        if (e instanceof RazorpayCancelledError) {
          ...
          // Last-mile safety net: on some UPI intent flows the native SDK
          // returns an auth-error even though the bank actually captured
          // the payment (webhook then lands seconds later). Before showing
          // a scary toast, ask the server if enrollment already happened.
          // If yes → celebrate. If no → fall back to friendly copy so the
          // user knows the webhook will still enrol them if money moved.
          toast.info("Confirming with server…");
          const reconciled = await attemptReconcile(Number(courseId));
          if (reconciled) {
            playSuccessSound();
            void notifySuccess();
          ...
          toast.error(
            `${formatRazorpayError({
              ...
              step: /authent/i.test(e?.message || "") ? "payment_authentication" : undefined,
              reason: e?.reason,
              code: e?.code,
            })} If money was debited, enrollment will happen automatically within a few minutes.`
          );
        }
      }
      return;
