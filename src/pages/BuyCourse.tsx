...
import type { RazorpayOrderResponse } from "../utils/paymentTypes";
/** Fields this screen reads off the fetched course row. */
type BuyCourseRow = CourseRow & {
  title?: string;
  imageUrl?: string;
...
const BuyCourse = () => {
  ...
  const [course, setCourse] = useState<BuyCourseRow | null>(null);
  ...
  const handleFreeEnrollment = async (courseIdNum: number) => {
    ...
    try {
      ...
      // Client writes to `enrollments` are revoked by design (paid-bypass
      // hardening). Free enrollment goes through the server-authoritative
      // `self-enroll-free` edge function, which re-checks the price with the
      // service role and upserts idempotently.
      const { data: result, error } = await supabase.functions.invoke("self-enroll-free", {
        body: { course_id: courseIdNum },
      });

      if (error) throw error;
      if (!result?.enrolled) throw new Error(result?.error || "ENROLL_FAILED");
  ...
  const handleRazorpayPayment = async (opts?: { forceWeb?: boolean }) => {
    ...
    try {
      orderData = await invokePaymentFunction<RazorpayOrderResponse>("create-razorpay-order", {
    ...
    if (Capacitor.isNativePlatform() && !opts?.forceWeb) {
      try {
        // Razorpay builds its UPI / "Recommended" block from the customer's
        // phone number. Without it the native sheet opens on a contact-entry
        // step and no UPI app tiles render — tell the user how to skip that.
        if (!sharedOpts.prefill?.contact) {
          toast.info("Profile me mobile number add karoge to UPI apps seedha checkout me dikhenge.", {
            action: { label: "Profile", onClick: () => navigate("/profile") },
          });
        }
        void tapMedium();
        ...
        if (e instanceof RazorpayCancelledError) {
          ...
          void notifyError();
          toast.error(formatRazorpayError({ description: getErrorMessage(e, "") })
            + " If your money was deducted, enrollment will happen automatically.");
        }
      } finally {
    ...
    try {
      await openRazorpayCheckout({
        ...
        onFailure: (err) => {
          ...
          toast.error(formatRazorpayError(err)
            + " If your money was deducted, enrollment will happen automatically via webhook.");
        },
        modal: {
          ondismiss: () => {
            toast.info("Payment cancelled. You can try again whenever you're ready.");
      ...
      toast.error((getErrorMessage(error, "") || "Failed to open checkout. Please try again.")
        + " If your money was deducted, enrollment will happen automatically via webhook.");
    }
  };
  ...
  const verifyRazorpayPayment = async (response: RazorpaySuccessResponse) => {
    try {
      ...
      const unreachable =
        getErrorMessage(error, "") === "razorpay_unreachable" || (error as { status?: number })?.status === 503;
      ...
      if ((error as { code?: string })?.code === "TIMEOUT" || unreachable || ((error as { status?: number })?.status && (error as { status?: number }).status >= 500)) {
