...
import type { CourseRow, LessonRow } from "@/types/rows";
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        (lessonsRes.data || []).forEach((l: LessonRow) => {
          if (l.course_id) countMap[l.course_id] = (countMap[l.course_id] || 0) + 1;
        });

        let filtered = coursesRes.data || [];
        if (selectedBatch) {
          filtered = filtered.filter((c: CourseRow) => c.id === selectedBatch.id);
        }

        const formattedCourses: Course[] = filtered.map((c: CourseRow) => ({
          id: c.id,
          title: c.title,
          grade: c.grade,
  ...
  useEffect(() => {
    ...
    const fetchResources = async () => {
      ...
      try {
        ...
        const mapLesson = (l: LessonRow): Resource => ({
          id: l.id,
          title: l.title,
        ...
        if (selectedBatch) {
          const filtered = (lessonsData || []).filter((l: LessonRow) => l.course_id === selectedBatch.id);
