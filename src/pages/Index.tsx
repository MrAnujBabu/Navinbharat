    ...
    >
      <div className="container mx-auto max-w-7xl px-6 lg:px-10 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated ? (
            <Link to="/dashboard" onClick={() => { void tapHaptic("light"); }}>
              <Button className="h-10 px-5 rounded-md text-sm font-medium active:scale-[0.97] transition-transform duration-150 ease-out">Dashboard</Button>
            </Link>
          ) : (
            <>
              <Link to="/login" onClick={() => { void selectionHaptic(); }} className={linkCls}>Login</Link>
              <Link to="/signup" onClick={() => { void tapHaptic("light"); }}>
                <Button className="h-10 px-5 rounded-md text-sm font-medium active:scale-[0.97] transition-transform duration-150 ease-out">Sign up free</Button>
              </Link>
            </>
          )}
        </div>
        ...
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          <SheetTrigger asChild className="md:hidden">
            <Button variant="ghost" size="icon" aria-label="Open menu" onClick={() => { void selectionHaptic(); }} className="active:scale-[0.94] transition-transform duration-150 ease-out">
          ...
          <SheetContent side="right" className="w-72">
            ...
            <div className="flex flex-col gap-1 mt-8">
              <Link to="/courses" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                <Button variant="ghost" className="h-11 w-full justify-start active:scale-[0.98] transition-transform duration-150 ease-out">Courses</Button>
              </Link>
              <Link to="/books" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                <Button variant="ghost" className="h-11 w-full justify-start active:scale-[0.98] transition-transform duration-150 ease-out">Resources</Button>
              </Link>
              <a href="#why-choose-us" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                <Button variant="ghost" className="h-11 w-full justify-start active:scale-[0.98] transition-transform duration-150 ease-out">About</Button>
              </a>
              <div className="h-px bg-border my-4" />
              {isAuthenticated ? (
                <Link to="/dashboard" onClick={() => { void tapHaptic("light"); setMobileOpen(false); }}>
                  <Button className="h-11 w-full active:scale-[0.97] transition-transform duration-150 ease-out">Dashboard</Button>
                </Link>
              ) : (
                <>
                  <Link to="/login" onClick={() => { void selectionHaptic(); setMobileOpen(false); }}>
                    <Button variant="outline" className="h-11 w-full active:scale-[0.97] transition-transform duration-150 ease-out">Login</Button>
                  </Link>
                  <Link to="/signup" onClick={() => { void tapHaptic("light"); setMobileOpen(false); }}>
                    <Button className="h-11 w-full mt-2 active:scale-[0.97] transition-transform duration-150 ease-out">Sign up free</Button>
                  </Link>
                </>
              )}
            </div>
      ...
      {!authState && (
        ...
        >
          <Link to="/login" onClick={() => { void selectionHaptic(); }} className="flex-1">
            <Button variant="outline" className="w-full h-11 text-sm font-medium rounded-md active:scale-[0.97] transition-transform duration-150 ease-out">
              Login
            </Button>
          </Link>
          <Link to="/signup" onClick={() => { void tapHaptic("light"); }} className="flex-1">
            <Button className="w-full h-11 text-sm font-medium rounded-md active:scale-[0.97] transition-transform duration-150 ease-out">
              Sign up free
            </Button>
          </Link>
        </div>
