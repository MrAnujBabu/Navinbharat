...
import type { ChapterRow, LessonRow, QuizRow } from "@/types/rows";
...
const toLesson = (l: LessonRow, idx: number): Lesson => ({
  id: String(l.id ?? ""),
  title: l.title ?? "",
  video_url: l.video_url ?? "",
  description: l.description ?? null,
  is_locked: l.is_locked ?? null,
...
const LectureListing = () => {
  ...
  const [standaloneQuizzes, setStandaloneQuizzes] = useState<QuizRow[]>([]);
        ...
        if (chapterId && chapterId !== "__all__") {
          ...
          if (!chapterError && chapterData) {
            ...
            if (subChaptersData && subChaptersData.length > 0) {
              ...
              setLessons(combined.map(toLesson));
            } else {
              ...
              const { data: lessonsData, error: lessonsError } = await supabase
                .from("lessons").select("id,title,video_url,course_id,chapter_id,lecture_type,position,thumbnail_url,class_pdf_url,is_locked,duration,duration_seconds,youtube_id,created_at,overview,description,like_count,is_free,is_preview,file_path,category").eq("chapter_id", chapterId).order("position", { ascending: true }).limit(200);
              if (!lessonsError) {
                setLessons((lessonsData || []).map(toLesson));
              }
            }
          }
      } else {
          ...
          if (allChapters && allChapters.length > 0) {
            ...
            (nestedChapters || []).forEach((nc: ChapterRow) => { if (nc.parent_id) parentMap[nc.id] = nc.parent_id; });
            
            const countMap: Record<string, number> = {};
            (chapterLessonsData || []).forEach((l: LessonRow) => {
  ...
  useEffect(() => {
    ...
    const fetchQuizzes = async () => {
      ...
      if (lessonIds.length > 0) {
        ...
        if (data) {
          data.forEach((q: QuizRow) => { if (q.lesson_id) map[q.lesson_id] = q.id; });
        }
      }
    ...
    fetchQuizzes();
  // eslint-disable-next-line react-hooks/exhaustive-deps -- chapterId is applied by this effect's own filter reset; including it would fight the URL sync.
  }, [lessons, courseId]);
