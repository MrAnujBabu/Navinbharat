...
import type { EnrollmentRow } from "@/types/rows";
...
const teacherFeatures = [
  { icon: ClipboardCheck, label: "Attendance", color: "bg-primary/10 text-primary", path: "/attendance" },
  { icon: FileText, label: "Report Card", color: "bg-accent/10 text-accent", path: "/reports" },
  { icon: Users, label: "Students", color: "bg-primary/10 text-primary", path: "/students" },
  { icon: Calendar, label: "Timetable", color: "bg-accent/10 text-accent", path: "/timetable" },
];
...
interface DashboardSnapshot {
  ...
  upcoming_doubts?: { id: string; subject: string; scheduled_at: string; zoom_join_url: string; status: string }[];
}

const isPermissionDenied = (error: { code?: string; message?: string } | null) =>
  error?.code === "42501" ||
...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (enrollments.length > 0) {
          ...
          const enrolledRaw = enrollments
            .filter((e: EnrollmentRow) => {
            ...
            .map((e: EnrollmentRow) => {
              ...
              // Count distinct lessons that still belong to this course, so
              // orphaned progress rows can't push the percentage past 100%.
              const completedIds = new Set<string>();
              progressData.forEach((p) => {
                if (p.completed && courseLessonIds.has(p.lesson_id)) completedIds.add(p.lesson_id);
              });
              const total = courseLessons.length;
              const completedCount = Math.min(completedIds.size, total);
              const pct = total > 0
                ? Math.min(100, Math.max(0, Math.round((completedCount / total) * 100)))
                : 0;
              return {
                id: courseId,
          ...
          // The `content` bucket is private: legacy `/object/public/content/...`
          // URLs stored on courses 404 ("Bucket not found") unless signed. The
          // snapshot RPC returns them raw, so sign them here the same way
          // useEnrollments does — otherwise the resume card renders a grey box.
          const safeResolve = async (u?: string | null) => {
            if (!u) return null;
            try { return await resolveContentUrl(u); } catch { return null; }
          };
          const enrolled = await Promise.all(
            enrolledRaw.map(async (c: DashboardCourse) => ({
              ...c,
              imageUrl: await safeResolve(c.imageUrl),
              thumbnailUrl: await safeResolve(c.thumbnailUrl),
            }))
          );
          if (!alive) return;
        ...
        if ((error as { name?: string })?.name === "AbortError" || !alive) return;
        ...
        // cold session.
        const msg = typeof getErrorMessage(error, "") === "string" ? getErrorMessage(error, "") : "";
  ...
  return (
    <div className="min-h-dvh bg-muted/30 flex flex-col font-sans">
      ...
      {/* pb-28 on mobile keeps the floating chat button from covering the last
          row of quick actions (it sat on top of "My Doubts"). */}
      <main className="flex-1 overflow-y-auto px-4 md:px-6 pb-28 md:pb-6 space-y-4 md:space-y-6 max-w-7xl mx-auto w-full pt-3 md:pt-4">
