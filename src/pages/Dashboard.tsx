...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (enrollments.length > 0) {
          ...
          const enrolledWithSignedImages = await Promise.all(
            enrolled.map(async (course) => ({
              ...course,
              imageUrl: (await resolveContentUrl(course.imageUrl)) ?? course.imageUrl,
              thumbnailUrl: (await resolveContentUrl(course.thumbnailUrl)) ?? course.thumbnailUrl,
            }))
          );
          setMyCourses(enrolledWithSignedImages);
          setProgressPercent(enrolledWithSignedImages[0]?.progressPercent || 0);
        }
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex flex-col font-sans">
      ...
      <main className="flex-1 overflow-y-auto px-4 md:px-6 pb-20 md:pb-6 space-y-4 md:space-y-6 max-w-7xl mx-auto w-full pt-[calc(52px+env(safe-area-inset-top)+0.25rem)] md:pt-[calc(60px+env(safe-area-inset-top)+1rem)]">
              ...
              >
                <div className="flex flex-col sm:flex-row">
                  <div className="sm:w-48 h-36 sm:h-auto bg-muted relative overflow-hidden flex-shrink-0">
                    <SmartImage
                      src={myCourses[0].thumbnailUrl || myCourses[0].imageUrl || coursePlaceholder} 
