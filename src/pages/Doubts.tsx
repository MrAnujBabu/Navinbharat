    ...
    try {
      ...
      if (newSession?.id) {
        try {
          ...
          const resp = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/resolve-doubt`, {
            method: "POST",
            headers: {
  ...
  const handleCreateZoomMeeting = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-zoom-meeting`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <div className="flex flex-row sm:flex-col gap-2 shrink-0">
            ...
            {session.zoom_join_url && !onJoin && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => window.open(session.zoom_join_url!, "_blank", "noopener,noreferrer")}
                className="gap-1"
              >
