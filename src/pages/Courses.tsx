...
import type { CourseRow, EnrollmentRow } from "@/types/rows";
...
const Courses = () => {
  ...
  const { data: courseList = [], isLoading: loading } = useQuery({
    ...
    queryFn: async (): Promise<CourseProps[]> => {
      ...
      const signed = await resolveContentUrls(
        rows.map((item: CourseRow) => item.image_url || item.thumbnail_url || null),
      );

      return rows.map((item: CourseRow, i: number) => ({
        id: item.id,
        title: item.title,
  ...
  const fetchEnrollments = async () => {
    ...
    try {
      ...
      if (!error && data) {
        setEnrolledCourseIds(new Set(data.map((e: EnrollmentRow) => e.course_id)));
      }
    } catch (error) {
