...
// Canonical asset is NaveenBharat.apk, published to the releases of
// github.com/MrAnujBabu/Naveen-Bharat-Install. The page ALWAYS resolves the
// newest release through the API first, so a new upload goes live here with no
// code change; the fixed `/releases/latest/download/` URL is only the fallback
// for when the API is unreachable (rate limit / offline).
const APK_REPO = "MrAnujBabu/Naveen-Bharat-Install";
const APK_ASSET_NAME = "NaveenBharat.apk";
...
// Bumped v3 → v4 with the repo move to Naveen-Bharat-Install: v3 entries hold
// the old MrAnujBabu/NaveenBharat URL and would keep serving it for up to 6h.
const APK_CACHE_KEY = "nb:latest_apk:v4";
// Short TTL: a fresh release should reach visitors within the hour, and the
// cache only exists so the first paint has a trusted URL before the API answers.
const APK_CACHE_TTL_MS = 60 * 60 * 1000; // 1h
...
async function fetchLatestApk(signal: AbortSignal): Promise<ApkInfo | null> {
  try {
    ...
    const json = (await res.json()) as { assets?: GithubReleaseAsset[]; tag_name?: string };
    ...
    // NaveenBharat*.apk, and only then to whatever .apk the release carries.
    const asset =
      assets.find((a) => isApk(a) && a.name === APK_ASSET_NAME) ??
      assets.find((a) => isApk(a) && /^naveenbharat/i.test(a.name)) ??
      assets.find(isApk);
...
/** Subset of a GitHub release asset we read when resolving the APK link. */
...
const Install = () => {
  ...
  // Resolve the newest APK from the GitHub Releases API on mount, and again
  // whenever the tab comes back to the foreground — a long-open install page
  // must never hand out a stale build. Silent failure: the cached/fallback URL
  // is already in state, so the button always works.
  useEffect(() => {
    const ctrl = new AbortController();
    const resolve = () => {
      void fetchLatestApk(ctrl.signal).then((info) => {
    ...
    <main className="min-h-dvh bg-background">
      ...
      >
        <div className="container mx-auto px-4 py-3 flex items-center gap-3 max-w-2xl">
          ...
          <img src={appLogo} alt="Naveen Bharat" className="h-9 w-9 rounded-xl object-cover" />
