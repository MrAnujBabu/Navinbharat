...
import type { CourseRow, LessonRow, ProfileRow, QuestionRow, QuizAttemptRow } from "@/types/rows";

type AttemptWithStudent = QuizAttemptRow & { student_name: string };
...
const AdminQuizManager = () => {
  ...
  const handleCreateQuiz = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("quizzes").insert(payload as never).select().single();
  ...
  const loadQuizForEdit = async (quiz: Quiz) => {
    ...
    if (data && data.length > 0) {
      setQuestionForms(data.map((q: QuestionRow) => ({
        ...
        question_type: (q.question_type as QuestionForm["question_type"]) ?? "mcq",
  ...
  const openAttempts = async (quiz: Quiz) => {
    ...
    const userIds = [...new Set((data || []).map((a: QuizAttemptRow) => a.user_id as string))];
    ...
    if (userIds.length > 0) {
      ...
      (profiles || []).forEach((p: ProfileRow) => {
        profileMap[String(p.id)] = p.full_name || p.email || "Unknown";
      });
    }

    const enriched: AttemptWithStudent[] = (data || []).map((a: QuizAttemptRow) => ({
      ...a,
      student_name: profileMap[String(a.user_id)] || "Unknown",
    }));
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <Select value={q.question_type} onValueChange={v => updateQuestionForm(qIdx, "question_type", v)}>
