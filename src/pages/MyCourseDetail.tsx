...
import { SafeBoundary } from "../lib/safety";
...
import type { ChapterRow, CourseRow, LessonRow } from "@/types/rows";
...
type ContentType = "all" | "lectures" | "pdfs" | "dpp" | "notes" | "tests";
...
type CardLectureType = "VIDEO" | "PDF" | "DPP" | "NOTES" | "TEST";

/**
 * My Courses used to pass the raw `lecture_type` with a blanket `VIDEO`
 * fallback, so a document-only lesson rendered the video card labelled
 * "Lecture" with a Watch button — while the public course pages showed the
 * same lesson as "PDF". Derive the effective card type so both surfaces agree:
 * anything with no playable video but a PDF attachment IS a PDF.
 */
const effectiveLectureType = (lesson: {
  lectureType?: string | null;
  videoUrl?: string | null;
  youtubeId?: string | null;
  classPdfUrl?: string | null;
}, attachmentCount = 0): CardLectureType => {
  const raw = (lesson.lectureType || "").toUpperCase();
  if (raw === "PDF" || raw === "DPP" || raw === "NOTES" || raw === "TEST") return raw as CardLectureType;
  const hasVideo = !!(lesson.videoUrl?.trim() || lesson.youtubeId?.trim());
  const hasPdf = !!lesson.classPdfUrl?.trim() || attachmentCount > 0;
  if (!hasVideo && hasPdf) return "PDF";
  return "VIDEO";
};
...
const MyCourseDetail = () => {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    try {
      const v = localStorage.getItem("nb_lesson_view");
      // Migrate the legacy 2-way values onto the 3-way switcher.
      if (v === "card") return "gallery";
      if (v === "list" || v === "gallery" || v === "table") return v;
      return "list";
    } catch { return "list"; }
  });
  ...
  const courseQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      allChaptersList.forEach((ch: ChapterRow) => {
        if (ch.parent_id) {
      ...
      const mappedChapters: Chapter[] = allChaptersList.map((ch: ChapterRow) => {
  ...
  if (!course) {
    ...
    if (
      ...
      !!(courseQuery.data as { course?: unknown })?.course
    ) {
      return (
        <div className="min-h-dvh bg-background flex flex-col">
          <Header onMenuClick={() => setSidebarOpen(true)} userName={profile?.fullName || "User"} />
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto min-w-0 bg-background pb-[calc(env(safe-area-inset-bottom)+88px)] md:pb-0">
            ...
            <div className="hidden md:flex items-center gap-2 px-4 py-2">
              ...
              {selectedChapterId && !selectedLesson && (
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => { setSearchOpen((o) => !o); if (searchOpen) setLessonSearch(""); }}
                    aria-label={searchOpen ? "Close search" : "Search lessons"}
                  ...
                  <ContentViewSwitcher activeView={viewMode} onViewChange={handleViewModeChange} modes={["gallery", "list", "table"]} />
                </div>
              )}
              <button
              ...
              {selectedChapterId && !selectedLesson && (
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    aria-expanded={searchOpen}
                    className={cn(
                      "inline-flex items-center justify-center h-8 w-8 rounded-lg transition-colors active:scale-95",
                      searchOpen ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted"
                    )}
                  >
                    <Search className="h-4 w-4" />
                  </button>
          ...
          {selectedChapterId && !selectedLesson && (() => {
            ...
            return (
              <>
                {/* ── Completion banner — compact linear progress (mobile-first) ── */}
                {totalInChapter > 0 && (
                  <div className={cn(
                    "mx-5 mt-3 px-3 py-2 rounded-xl border transition-all",
                    isAllDone
                      ? "bg-green-50 border-green-200 dark:bg-green-950/20 dark:border-green-800/40"
                      : "bg-primary/5 border-primary/10"
                  )}>
                    <div className="flex items-center gap-2">
                      {isAllDone && <Trophy className="h-4 w-4 text-green-500 shrink-0" />}
                      <p className={cn(
                        "flex-1 min-w-0 truncate text-[13px] font-medium",
                        isAllDone ? "text-green-700 dark:text-green-400" : "text-foreground"
                      )}>
                        {isAllDone
                          ? "Chapter complete"
                ...
                {chapterLessons.length > 0 && (
                  <>
                        ...
                        {tabs.map((tab) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "shrink-0 rounded-full border px-4 py-1.5 text-sm font-medium transition-all whitespace-nowrap",
                                isActive
                    ...
                    <div className="px-5 pb-1 pt-1">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-9 pr-8 py-2 text-base bg-muted rounded-xl border-0 outline-none focus:ring-2 focus:ring-primary/50 placeholder:text-muted-foreground"
                        />
                        {lessonSearch && (
                          <button
                    ...
                    <div className={cn("p-5", viewMode === "list" ? "space-y-3" : "space-y-4")}>
                      ...
                      {filteredLessons.length === 0 ? (
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {filteredLessons.map((lesson) => (
                            <LectureGalleryCard
                              ...
                              id={lesson.id}
                              title={lesson.title}
                              lectureType={effectiveLectureType(lesson, attachmentCounts[lesson.id] || 0)}
                              ...
                              onClick={() => handleRowSelect(lesson.id)}
                            />
                          ))}
                        </div>
                      ) : viewMode === "table" ? (
                        <LectureTableView
                          lessons={filteredLessons.map((lesson) => ({
                            id: lesson.id,
                            title: lesson.title,
                            lecture_type: effectiveLectureType(lesson, attachmentCounts[lesson.id] || 0),
                          ...
                          onLectureClick={(lesson) => handleRowSelect(lesson.id)}
                        />
                      ) : (
      ...
      {/* Own boundary: a crash inside the PDF/notes reader must not take the
          whole course page down with it (safe-surface-handling). */}
      <SafeBoundary
        fallbackTitle="PDF khul nahi paaya"
        fallback={null}
      >
