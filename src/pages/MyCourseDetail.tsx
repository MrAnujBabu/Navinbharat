...
const MyCourseDetail = () => {
  ...
  const courseQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      // Perf: consolidate courses+chapters+lessons+enrollment into one RPC
      // call (get_course_bundle). Cuts the chatty `lessons WHERE course_id=…`
      // hot path (~10k calls/day) to a single round-trip per course view.
      const [bundleRes, progressRes] = await Promise.all([
        supabase.rpc("get_course_bundle", { _course_id: cid }),
        user?.id
      ...
      const bundle = (bundleRes.data ?? {}) as {
        course: { id: number; title: string; description: string | null; grade: string | null; image_url: string | null; thumbnail_url: string | null } | null;
        chapters: Array<{ id: string; code: string | null; title: string; parent_id: string | null; position: number | null; thumbnail_url: string | null }>;
        lessons: Array<Record<string, any>>;
        is_enrolled: boolean;
      };
      if (!bundle.course) throw new Error("Course not found");

      const cd = bundle.course;
      const courseObj: Course = {
        id: cd.id, title: cd.title, description: cd.description,
        grade: cd.grade, imageUrl: cd.image_url, thumbnailUrl: cd.thumbnail_url,
      };

      const mappedLessons: Lesson[] = (bundle.lessons || []).map((l: any, idx: number) => ({
      ...
      const allChaptersList = bundle.chapters || [];
      ...
      const result = {
        course: courseObj,
        hasPurchased: bundle.is_enrolled,
