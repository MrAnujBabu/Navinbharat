...
app.post("/api/functions/v1/score-quiz", async (req, res) => {
  ...
  const admin = makeAdminClient();
  const { data: quiz, error: quizError } = await admin.from("quizzes").select("id, total_marks, pass_percentage").eq("id", quiz_id).single();
...
app.post("/api/functions/v1/resolve-doubt", async (req, res) => {
  ...
  let session = null;
  if (!isLessonChat) {
    const { data } = await admin.from("doubt_sessions").select("student_id").eq("id", sessionId).single();
    session = data;
  ...
  if (isLessonChat) {
    ...
    systemPrompt = "You are Naveen AI Agent, a helpful teaching assistant for Naveen Bharat coaching. Answer the student's doubt clearly and concisely in Hindi or English based on the question language. Give step-by-step explanation if needed. Keep it under 500 words." + ragContext;
  }
...
app.post("/api/functions/v1/summarize-video", async (req, res) => {
  ...
  if (normalizedDescription) contextBlock += `\n\n**Lesson Description:**\n${normalizedDescription}`;
  if (normalizedOverview) contextBlock += `\n\n**Topics/Overview:**\n${normalizedOverview}`;
  ...
  let systemPrompt = `You are Naveen Sarthi, the AI learning companion for Naveen Bharat coaching platform.\n${groundingInstruction}`;
  let prompt;

  if (mode === "research") {
    systemPrompt = `You are Naveen Sarthi, an expert educational researcher for Naveen Bharat platform.\n${groundingInstruction}`;
...
app.post("/api/functions/v1/get-lesson-url", async (req, res) => {
  ...
  const { data: enrollment } = await admin.from("enrollments").select("id").eq("user_id", user.id).eq("course_id", lesson.course_id).eq("status", "active").maybeSingle();
