...
// SECURITY 2026-08-19: the legacy in-process refund implementation was removed.
// It called the Razorpay refund API straight after the admin-role check, with no
// lookup of the local `razorpay_payments` row, no double-refund guard, no
// refundable-status check and no audit_log entry — all of which the Supabase
// edge function `initiate-refund` enforces. Refunds are irreversible external
// side effects, so this duplicate weaker path is gone for good. Nothing in the
// app calls it (the admin UI uses `supabase.functions.invoke("initiate-refund")`).
app.post("/api/functions/v1/initiate-refund", (_req, res) => {
  res.status(410).json({
    success: false,
    error: "This endpoint has been removed. Refunds go through the initiate-refund edge function.",
  });
});
