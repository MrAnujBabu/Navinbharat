...
3. **Redeploy the App Link host.** The only origin in `APP_LINK_HOSTS`
   (`src/config/deepLinks.ts`) is `navinbharat.vercel.app`, and it must serve
...
| --- | --- | --- |
| Vercel web deploy | project env `ANDROID_CERT_SHA256` | **hard failure** when `VERCEL_ENV=production`; Preview/Development log a one-line `ℹ️` note and continue |
...
| Other GitHub Actions builds (manual APK, signed-apk smoke, Maestro, Playwright, Lighthouse) | repo secret `ANDROID_CERT_SHA256`, passed as step env | info note, build continues |
| `code-guards` deep-link guard | repo secret + `ASSETLINKS_STRICT=1` | warning, fingerprint check advisory |
| Local `bun run build` | shell env | info note, build continues |

> Set `ANDROID_CERT_SHA256` on **all three** Vercel scopes (Production, Preview,
> Development). With Production only, Preview/Development deploys keep printing
> the `ℹ️ assetlinks: … not set` note — harmless, but noisy in build logs.

### Node version pin

`scripts/check-node-pin.mjs` (in `guard:all` + `code-guards.yml`) fails if
`package.json` `engines.node` becomes an open-ended range (`>=22`, `^22`, `*`)
or drifts from `.node-version` / `.nvmrc` / `.tool-versions`. Vercel warns on
open ranges because it silently upgrades to the next Node major, which can break
a build with no code change. Current pin: `22.x`.
...
```bash
curl -s https://navinbharat.vercel.app/.well-known/assetlinks.json | jq .
