...
| # | Item | Status | Notes |
|---|---|---|---|
| 1 | Maestro device-matrix flows | ✅ done | New `.github/workflows/maestro-on-apk.yml` — chains via `workflow_run` on parent build success, runs `smoke.yaml + pdf-back.yaml + back-button-cold-start.yaml + offline-queue.yaml` on API 26 / 30 / 34 emulators, uploads logcat + junit XML on failure. |
